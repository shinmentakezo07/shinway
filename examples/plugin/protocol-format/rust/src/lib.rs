...
#[repr(C)]
pub struct ShinwayBuffer {
    ptr: *mut u8,
    len: usize,
}

type HostCall = unsafe extern "C" fn(*mut std::ffi::c_void, *const c_char, *const u8, usize, *mut ShinwayBuffer) -> i32;
type HostFree = unsafe extern "C" fn(*mut std::ffi::c_void, usize);
type PluginCall = unsafe extern "C" fn(*const c_char, *const u8, usize, *mut ShinwayBuffer) -> i32;
type PluginFree = unsafe extern "C" fn(*mut std::ffi::c_void, usize);
type PluginShutdown = unsafe extern "C" fn();

#[repr(C)]
pub struct ShinwayHostApi {
    abi_version: u32,
...
static mut STORED_HOST: *const ShinwayHostApi = ptr::null();

#[no_mangle]
pub extern "C" fn shinway_plugin_init(host: *const ShinwayHostApi, plugin: *mut ShinwayPluginApi) -> i32 {
    if plugin.is_null() {
        return 1;
    }
    unsafe {
...
unsafe extern "C" fn plugin_call(method: *const c_char, request: *const u8, request_len: usize, response: *mut ShinwayBuffer) -> i32 {
    if !response.is_null() {
    ...
    let _ = request_len;
    match method {
        "plugin.register" => {            write_response(response, "{\"ok\":true,\"result\":{\"schema_version\":1,\"metadata\":{\"Name\":\"example-protocol-format-rust\",\"Version\":\"0.1.0\",\"Author\":\"shinmentakezo07\",\"GitHubRepository\":\"https://github.com/shinmentakezo07/shinway\",\"Logo\":\"https://example.invalid/example-protocol-format-rust.png\",\"ConfigFields\":[]},\"capabilities\":{\"executor\":true,\"executor_model_scope\":\"both\",\"executor_input_formats\":[\"chat-completions\"],\"executor_output_formats\":[\"responses\"]}}}"); 0 },"plugin.reconfigure" => {            write_response(response, "{\"ok\":true,\"result\":{\"schema_version\":1,\"metadata\":{\"Name\":\"example-protocol-format-rust\",\"Version\":\"0.1.0\",\"Author\":\"shinmentakezo07\",\"GitHubRepository\":\"https://github.com/shinmentakezo07/shinway\",\"Logo\":\"https://example.invalid/example-protocol-format-rust.png\",\"ConfigFields\":[]},\"capabilities\":{\"executor\":true,\"executor_model_scope\":\"both\",\"executor_input_formats\":[\"chat-completions\"],\"executor_output_formats\":[\"responses\"]}}}"); 0 },"executor.identifier" => {            write_response(response, "{\"ok\":true,\"result\":{\"identifier\":\"example-protocol-format-rust\"}}"); 0 },"executor.execute" => {            write_response(response, "{\"ok\":true,\"result\":{\"Payload\":\"eyJpZCI6ImV4YW1wbGUtcHJvdG9jb2wtZm9ybWF0LXJ1c3QiLCJvYmplY3QiOiJjaGF0LmNvbXBsZXRpb24ifQ==\",\"Headers\":{\"content-type\":[\"application/json\"]}}}"); 0 },
        _ => {
...
fn write_response(response: *mut ShinwayBuffer, text: &str) {
    if response.is_null() {
        return;
    }
...
fn call_host(method: &str, payload: &str) {
    unsafe {
        ...
        method_bytes.push(0);
        let mut response = ShinwayBuffer { ptr: ptr::null_mut(), len: 0 };
        let rc = call(
            host.host_ctx,
