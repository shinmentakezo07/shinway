...
static int plugin_call(const char* method, const uint8_t* request, size_t request_len, shinway_buffer* response) {
	...
	if (strcmp(method, "plugin.register") == 0) {
		write_response(response, "{\"ok\":true,\"result\":{\"schema_version\":1,\"metadata\":{\"Name\":\"example-auth-c\",\"Version\":\"0.1.0\",\"Author\":\"shinmentakezo07\",\"GitHubRepository\":\"https://github.com/shinmentakezo07/shinway\",\"Logo\":\"https://example.invalid/example-auth-c.png\",\"ConfigFields\":[]},\"capabilities\":{\"auth_provider\":true}}}");
		return 0;
	}
