...
This directory is the reference skeleton for the current standard dynamic library plugin ABI. The ABI is language-neutral: the host loads a native dynamic library, calls `shinway_plugin_init`, and then exchanges JSON envelopes through a stable C function table.
...
```c
int shinway_plugin_init(const shinway_host_api* host, shinway_plugin_api* plugin);
```

The plugin fills `shinway_plugin_api` with:

```c
int call(char* method, uint8_t* request, size_t request_len, shinway_buffer* response);
void free_buffer(void* ptr, size_t len);
void shutdown(void);
```

The host provides `shinway_host_api` with:

```c
int call(void* host_ctx, char* method, uint8_t* request, size_t request_len, shinway_buffer* response);
void free_buffer(void* ptr, size_t len);
```
...
```bash
mkdir -p plugins/darwin/$(go env GOARCH)
cmake -S examples/plugin/simple/c -B /tmp/shinway-simple-c-build -DCMAKE_LIBRARY_OUTPUT_DIRECTORY=$PWD/plugins/darwin/$(go env GOARCH)
cmake --build /tmp/shinway-simple-c-build
```
...
CARGO_TARGET_DIR=/tmp/shinway-simple-rust-target cargo build --release --locked
cp /tmp/shinway-simple-rust-target/release/libshinway_simple_rust.dylib ../../../../plugins/darwin/$(go env GOARCH)/simple-rust.dylib
```
