...
This plugin demonstrates the shinway C ABI scheduler capability from Go.
...
```bash
cd go
go build -buildmode=c-shared -o /tmp/shinway-scheduler-plugin.so .
rm -f /tmp/shinway-scheduler-plugin.so /tmp/shinway-scheduler-plugin.h
```
