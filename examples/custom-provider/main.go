...
func main() {
	...
	hooks := shinway.Hooks{
		OnAfterStart: func(s *shinway.Service) {
			// Register demo models for the custom provider so they appear in /v1/models.
			models := []*shinway.ModelInfo{{ID: "myprov-pro-1", Object: "model", Type: providerKey, DisplayName: "MyProv Pro 1"}}
			for _, a := range core.List() {
				if strings.EqualFold(a.Provider, providerKey) {
					shinway.GlobalModelRegistry().RegisterClient(a.ID, providerKey, models)
				}
			}
		},
	}
