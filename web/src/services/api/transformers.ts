...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  const zenList = raw['zen-api-key'];
  if (Array.isArray(zenList)) {
    config.zenApiKeys = zenList
      .map((item) => normalizeProviderKeyConfig(item))
      .filter(Boolean) as ProviderKeyConfig[];
  }
  ...
  const orcaRouterList = raw['orcarouter-api-key'];
  if (Array.isArray(orcaRouterList)) {
    config.orcaRouterApiKeys = orcaRouterList
      .map((item) => normalizeProviderKeyConfig(item))
      .filter(Boolean) as ProviderKeyConfig[];
  }
