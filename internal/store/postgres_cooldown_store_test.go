...
func TestPostgresCooldownStateStore_SaveLoad(t *testing.T) {
	...
	driverName := fmt.Sprintf("shinway_postgres_cooldown_test_%d", cooldownTestDriverID.Add(1))
	...
	records := []shinwayauth.CooldownStateRecord{
		{
			Provider:       "codex",
			AuthID:         "account-1",
			Model:          "gpt-test",
			Status:         string(shinwayauth.StatusError),
	...
	zeroTimeRecord := shinwayauth.CooldownStateRecord{AuthID: "account-2", Model: "gpt-test"}
	if errSave := cooldownStore.Save(context.Background(), []shinwayauth.CooldownStateRecord{zeroTimeRecord}); errSave != nil {
...
func TestPostgresCooldownStateStore_MergesConcurrentInstances(t *testing.T) {
	state := &cooldownTestState{rows: make(map[string]cooldownTestRow)}
	driverName := fmt.Sprintf("shinway_postgres_cooldown_merge_test_%d", cooldownTestDriverID.Add(1))
	sql.Register(driverName, &cooldownTestDriver{state: state})
	db, errOpen := sql.Open(driverName, "")
	if errOpen != nil {
		t.Fatalf("sql.Open() error = %v", errOpen)
	...
	recordA := shinwayauth.CooldownStateRecord{AuthID: "account-a", Model: "model-a", UpdatedAt: updatedAt}
	recordB := shinwayauth.CooldownStateRecord{AuthID: "account-b", Model: "model-b", UpdatedAt: updatedAt}
	if errSave := storeA.Save(context.Background(), []shinwayauth.CooldownStateRecord{recordA}); errSave != nil {
		t.Fatalf("storeA.Save() error = %v", errSave)
	}
	if errSave := storeB.Save(context.Background(), []shinwayauth.CooldownStateRecord{recordB}); errSave != nil {
	...
	if errSave := storeA.Save(context.Background(), []shinwayauth.CooldownStateRecord{newerRecordA}); errSave != nil {
		t.Fatalf("storeA.Save(newer) error = %v", errSave)
	}
	if errSave := staleStore.Save(context.Background(), []shinwayauth.CooldownStateRecord{recordB}); errSave != nil {
