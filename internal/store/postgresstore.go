...
func (s *PostgresStore) Save(ctx context.Context, auth *shinwayauth.Auth) (string, error) {
	if auth == nil {
	...
	auth.Attributes[shinwayauth.AttributeSourceBackend] = shinwayauth.AuthSourcePostgres
...
func (s *PostgresStore) List(ctx context.Context) ([]*shinwayauth.Auth, error) {
	...
	auths := make([]*shinwayauth.Auth, 0, 32)
	for rows.Next() {
		var (
			id        string
			payload   string
		...
		attr := map[string]string{
			...
			shinwayauth.AttributeSourceBackend: shinwayauth.AuthSourcePostgres,
		}
...
func (s *PostgresStore) resolveAuthPath(auth *shinwayauth.Auth) (string, error) {
	if auth == nil {
		return "", fmt.Errorf("postgres store: auth is nil")
	}
	if auth.Attributes != nil {
