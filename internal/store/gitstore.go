...
func (s *GitTokenStore) Save(_ context.Context, auth *shinwayauth.Auth) (string, error) {
	if auth == nil {
	...
	auth.Attributes[shinwayauth.AttributeSourceBackend] = shinwayauth.AuthSourceGit
...
func (s *GitTokenStore) List(_ context.Context) ([]*shinwayauth.Auth, error) {
	if err := s.EnsureRepository(); err != nil {
		return nil, err
	}
...
func (s *GitTokenStore) readAuthFile(path, baseDir string) (*shinwayauth.Auth, error) {
	...
	auth := &shinwayauth.Auth{
		...
		Attributes: map[string]string{
			...
			shinwayauth.AttributeSourceBackend: shinwayauth.AuthSourceGit,
		},
		Metadata:         metadata,
...
func (s *GitTokenStore) resolveAuthPath(auth *shinwayauth.Auth) (string, error) {
	if auth == nil {
		return "", fmt.Errorf("auth filestore: auth is nil")
	}
	if auth.Attributes != nil {
