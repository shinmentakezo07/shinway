...
func (s *ObjectTokenStore) Save(ctx context.Context, auth *shinwayauth.Auth) (string, error) {
	if auth == nil {
	...
	auth.Attributes[shinwayauth.AttributeSourceBackend] = shinwayauth.AuthSourceObjectStore
...
func (s *ObjectTokenStore) List(_ context.Context) ([]*shinwayauth.Auth, error) {
	dir := strings.TrimSpace(s.AuthDir())
	if dir == "" {
		return nil, fmt.Errorf("object store: auth directory not configured")
	}
	entries := make([]*shinwayauth.Auth, 0, 32)
...
func (s *ObjectTokenStore) resolveAuthPath(auth *shinwayauth.Auth) (string, error) {
	if auth == nil {
		return "", fmt.Errorf("object store: auth is nil")
	}
	if auth.Attributes != nil {
...
func (s *ObjectTokenStore) readAuthFile(path, baseDir string) (*shinwayauth.Auth, error) {
	data, err := os.ReadFile(path)
	if err != nil {
	...
	attr := map[string]string{
		...
		shinwayauth.AttributeSourceBackend: shinwayauth.AuthSourceObjectStore,
	}
