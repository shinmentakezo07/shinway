...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	// NVIDIA NIM keys (do not print key material)
	if len(oldCfg.NVIDIAKey) != len(newCfg.NVIDIAKey) {
		changes = append(changes, fmt.Sprintf("nvidia-api-key count: %d -> %d", len(oldCfg.NVIDIAKey), len(newCfg.NVIDIAKey)))
	} else {
		for i := range oldCfg.NVIDIAKey {
			o := oldCfg.NVIDIAKey[i]
			n := newCfg.NVIDIAKey[i]
			if strings.TrimSpace(o.BaseURL) != strings.TrimSpace(n.BaseURL) {
				changes = append(changes, fmt.Sprintf("nvidia[%d].base-url: %s -> %s", i, formatURL(o.BaseURL), formatURL(n.BaseURL)))
			}
			if strings.TrimSpace(o.ProxyURL) != strings.TrimSpace(n.ProxyURL) {
				changes = append(changes, fmt.Sprintf("nvidia[%d].proxy-url: %s -> %s", i, formatProxyURL(o.ProxyURL), formatProxyURL(n.ProxyURL)))
			}
			if strings.TrimSpace(o.Prefix) != strings.TrimSpace(n.Prefix) {
				changes = append(changes, fmt.Sprintf("nvidia[%d].prefix: %s -> %s", i, strings.TrimSpace(o.Prefix), strings.TrimSpace(n.Prefix)))
			}
			if o.Priority != n.Priority {
				changes = append(changes, fmt.Sprintf("nvidia[%d].priority: %d -> %d", i, o.Priority, n.Priority))
			}
			if o.DisableCooling != n.DisableCooling {
				changes = append(changes, fmt.Sprintf("nvidia[%d].disable-cooling: %t -> %t", i, o.DisableCooling, n.DisableCooling))
			}
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {
				changes = append(changes, fmt.Sprintf("nvidia[%d].api-key: updated", i))
			}
			if !equalStringMap(o.Headers, n.Headers) {
				changes = append(changes, fmt.Sprintf("nvidia[%d].headers: updated", i))
			}
			oldModels := SummarizeCodexModels(o.Models)
			newModels := SummarizeCodexModels(n.Models)
			if oldModels.hash != newModels.hash {
				changes = append(changes, fmt.Sprintf("nvidia[%d].models: updated (%d -> %d entries)", i, oldModels.count, newModels.count))
			}
			oldExcluded := SummarizeExcludedModels(o.ExcludedModels)
			newExcluded := SummarizeExcludedModels(n.ExcludedModels)
			if oldExcluded.hash != newExcluded.hash {
				changes = append(changes, fmt.Sprintf("nvidia[%d].excluded-models: updated (%d -> %d entries)", i, oldExcluded.count, newExcluded.count))
			}
		}
	}

	// OpenCode Zen keys (do not print key material)
	if len(oldCfg.ZenKey) != len(newCfg.ZenKey) {
		changes = append(changes, fmt.Sprintf("zen-api-key count: %d -> %d", len(oldCfg.ZenKey), len(newCfg.ZenKey)))
	} else {
		for i := range oldCfg.ZenKey {
			o := oldCfg.ZenKey[i]
			n := newCfg.ZenKey[i]
			if strings.TrimSpace(o.BaseURL) != strings.TrimSpace(n.BaseURL) {
				changes = append(changes, fmt.Sprintf("zen[%d].base-url: %s -> %s", i, formatURL(o.BaseURL), formatURL(n.BaseURL)))
			}
			if strings.TrimSpace(o.ProxyURL) != strings.TrimSpace(n.ProxyURL) {
				changes = append(changes, fmt.Sprintf("zen[%d].proxy-url: %s -> %s", i, formatProxyURL(o.ProxyURL), formatProxyURL(n.ProxyURL)))
			}
			if strings.TrimSpace(o.Prefix) != strings.TrimSpace(n.Prefix) {
				changes = append(changes, fmt.Sprintf("zen[%d].prefix: %s -> %s", i, strings.TrimSpace(o.Prefix), strings.TrimSpace(n.Prefix)))
			}
			if o.Priority != n.Priority {
				changes = append(changes, fmt.Sprintf("zen[%d].priority: %d -> %d", i, o.Priority, n.Priority))
			}
			if o.DisableCooling != n.DisableCooling {
				changes = append(changes, fmt.Sprintf("zen[%d].disable-cooling: %t -> %t", i, o.DisableCooling, n.DisableCooling))
			}
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {
				changes = append(changes, fmt.Sprintf("zen[%d].api-key: updated", i))
			}
			if !equalStringMap(o.Headers, n.Headers) {
				changes = append(changes, fmt.Sprintf("zen[%d].headers: updated", i))
			}
			oldModels := SummarizeCodexModels(o.Models)
			newModels := SummarizeCodexModels(n.Models)
			if oldModels.hash != newModels.hash {
				changes = append(changes, fmt.Sprintf("zen[%d].models: updated (%d -> %d entries)", i, oldModels.count, newModels.count))
			}
			oldExcluded := SummarizeExcludedModels(o.ExcludedModels)
			newExcluded := SummarizeExcludedModels(n.ExcludedModels)
			if oldExcluded.hash != newExcluded.hash {
				changes = append(changes, fmt.Sprintf("zen[%d].excluded-models: updated (%d -> %d entries)", i, oldExcluded.count, newExcluded.count))
			}
		}
	}

	// TokenRouter keys (do not print key material)
	if len(oldCfg.TokenRouterKey) != len(newCfg.TokenRouterKey) {
		changes = append(changes, fmt.Sprintf("tokenrouter-api-key count: %d -> %d", len(oldCfg.TokenRouterKey), len(newCfg.TokenRouterKey)))
	} else {
		for i := range oldCfg.TokenRouterKey {
			o := oldCfg.TokenRouterKey[i]
			n := newCfg.TokenRouterKey[i]
			if strings.TrimSpace(o.BaseURL) != strings.TrimSpace(n.BaseURL) {
				changes = append(changes, fmt.Sprintf("tokenrouter[%d].base-url: %s -> %s", i, formatURL(o.BaseURL), formatURL(n.BaseURL)))
			}
			if strings.TrimSpace(o.ProxyURL) != strings.TrimSpace(n.ProxyURL) {
				changes = append(changes, fmt.Sprintf("tokenrouter[%d].proxy-url: %s -> %s", i, formatProxyURL(o.ProxyURL), formatProxyURL(n.ProxyURL)))
			}
			if strings.TrimSpace(o.Prefix) != strings.TrimSpace(n.Prefix) {
				changes = append(changes, fmt.Sprintf("tokenrouter[%d].prefix: %s -> %s", i, strings.TrimSpace(o.Prefix), strings.TrimSpace(n.Prefix)))
			}
			if o.Priority != n.Priority {
				changes = append(changes, fmt.Sprintf("tokenrouter[%d].priority: %d -> %d", i, o.Priority, n.Priority))
			}
			if o.DisableCooling != n.DisableCooling {
				changes = append(changes, fmt.Sprintf("tokenrouter[%d].disable-cooling: %t -> %t", i, o.DisableCooling, n.DisableCooling))
			}
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {
				changes = append(changes, fmt.Sprintf("tokenrouter[%d].api-key: updated", i))
			}
			if !equalStringMap(o.Headers, n.Headers) {
				changes = append(changes, fmt.Sprintf("tokenrouter[%d].headers: updated", i))
			}
			oldModels := SummarizeCodexModels(o.Models)
			newModels := SummarizeCodexModels(n.Models)
			if oldModels.hash != newModels.hash {
				changes = append(changes, fmt.Sprintf("tokenrouter[%d].models: updated (%d -> %d entries)", i, oldModels.count, newModels.count))
			}
			oldExcluded := SummarizeExcludedModels(o.ExcludedModels)
			newExcluded := SummarizeExcludedModels(n.ExcludedModels)
			if oldExcluded.hash != newExcluded.hash {
				changes = append(changes, fmt.Sprintf("tokenrouter[%d].excluded-models: updated (%d -> %d entries)", i, oldExcluded.count, newExcluded.count))
			}
		}
	}

	// OrcaRouter keys (do not print key material)
	if len(oldCfg.OrcaRouterKey) != len(newCfg.OrcaRouterKey) {
		changes = append(changes, fmt.Sprintf("orcarouter-api-key count: %d -> %d", len(oldCfg.OrcaRouterKey), len(newCfg.OrcaRouterKey)))
	} else {
		for i := range oldCfg.OrcaRouterKey {
			o := oldCfg.OrcaRouterKey[i]
			n := newCfg.OrcaRouterKey[i]
			if strings.TrimSpace(o.BaseURL) != strings.TrimSpace(n.BaseURL) {
				changes = append(changes, fmt.Sprintf("orcarouter[%d].base-url: %s -> %s", i, formatURL(o.BaseURL), formatURL(n.BaseURL)))
			}
			if strings.TrimSpace(o.ProxyURL) != strings.TrimSpace(n.ProxyURL) {
				changes = append(changes, fmt.Sprintf("orcarouter[%d].proxy-url: %s -> %s", i, formatProxyURL(o.ProxyURL), formatProxyURL(n.ProxyURL)))
			}
			if strings.TrimSpace(o.Prefix) != strings.TrimSpace(n.Prefix) {
				changes = append(changes, fmt.Sprintf("orcarouter[%d].prefix: %s -> %s", i, strings.TrimSpace(o.Prefix), strings.TrimSpace(n.Prefix)))
			}
			if o.Priority != n.Priority {
				changes = append(changes, fmt.Sprintf("orcarouter[%d].priority: %d -> %d", i, o.Priority, n.Priority))
			}
			if o.DisableCooling != n.DisableCooling {
				changes = append(changes, fmt.Sprintf("orcarouter[%d].disable-cooling: %t -> %t", i, o.DisableCooling, n.DisableCooling))
			}
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {
				changes = append(changes, fmt.Sprintf("orcarouter[%d].api-key: updated", i))
			}
			if !equalStringMap(o.Headers, n.Headers) {
				changes = append(changes, fmt.Sprintf("orcarouter[%d].headers: updated", i))
			}
			oldModels := SummarizeCodexModels(o.Models)
			newModels := SummarizeCodexModels(n.Models)
			if oldModels.hash != newModels.hash {
				changes = append(changes, fmt.Sprintf("orcarouter[%d].models: updated (%d -> %d entries)", i, oldModels.count, newModels.count))
			}
			oldExcluded := SummarizeExcludedModels(o.ExcludedModels)
			newExcluded := SummarizeExcludedModels(n.ExcludedModels)
			if oldExcluded.hash != newExcluded.hash {
				changes = append(changes, fmt.Sprintf("orcarouter[%d].excluded-models: updated (%d -> %d entries)", i, oldExcluded.count, newExcluded.count))
			}
		}
	}
