...
func applyCodexPromptCacheHeaders(from sdktranslator.Format, req shinwayexecutor.Request, rawJSON []byte) ([]byte, http.Header) {
	body, headers, _ := applyCodexPromptCacheHeadersWithContext(context.Background(), from, req, rawJSON)
	return body, headers
}

func applyCodexPromptCacheHeadersWithContext(ctx context.Context, from sdktranslator.Format, req shinwayexecutor.Request, rawJSON []byte, headerSets ...http.Header) ([]byte, http.Header, error) {
	headers := http.Header{}
	if len(rawJSON) == 0 {
...
func applyCodexWebsocketHeaders(ctx context.Context, headers http.Header, auth *shinwayauth.Auth, token string, cfg *config.Config) http.Header {
	if headers == nil {
		headers = http.Header{}
	}
...
func codexAuthUsesAPIKey(auth *shinwayauth.Auth) bool {
...
func codexHeaderDefaults(cfg *config.Config, auth *shinwayauth.Auth) (string, string) {
	if cfg == nil || auth == nil {
		return "", ""
	}
