...
func (e *OpenAICompatExecutor) HttpRequest(ctx context.Context, auth *shinwayauth.Auth, req *http.Request) (*http.Response, error) {
	if req == nil {
...
func (e *OpenAICompatExecutor) Execute(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (resp shinwayexecutor.Response, err error) {
...
func (e *OpenAICompatExecutor) executeImages(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options, endpointPath string) (resp shinwayexecutor.Response, err error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
	...
	reporter.Publish(ctx, helps.ParseOpenAIUsage(body))
	reporter.EnsurePublished(ctx)
	resp = shinwayexecutor.Response{Payload: body, Headers: httpResp.Header.Clone()}
	return resp, nil
}

func (e *OpenAICompatExecutor) ExecuteStream(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (_ *shinwayexecutor.StreamResult, err error) {
	if endpointPath := openAICompatImageEndpointPath(opts); endpointPath != "" {
		...
		for scanner.Scan() {
			...
			if !bytes.HasPrefix(trimmedLine, []byte("data:")) {
				...
				if bytes.HasPrefix(trimmedLine, []byte("{")) || bytes.HasPrefix(trimmedLine, []byte("[")) {
					...
					select {
					case out <- shinwayexecutor.StreamChunk{Err: streamErr}:
					case <-ctx.Done():
					}
					return
				}
			...
			for i := range chunks {
				select {
				case out <- shinwayexecutor.StreamChunk{Payload: chunks[i]}:
				case <-ctx.Done():
					return
				}
			}
...
func (e *OpenAICompatExecutor) executeImagesStream(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options, endpointPath string) (_ *shinwayexecutor.StreamResult, err error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
...
func (e *OpenAICompatExecutor) CountTokens(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
	...
	if err != nil {
		return shinwayexecutor.Response{}, fmt.Errorf("openai compat executor: tokenizer init failed: %w", err)
	}

	count, err := helps.CountOpenAIChatTokens(enc, translated)
	if err != nil {
		return shinwayexecutor.Response{}, fmt.Errorf("openai compat executor: token counting failed: %w", err)
	}

	usageJSON := helps.BuildOpenAIUsageJSON(count)
	translatedUsage := sdktranslator.TranslateTokenCount(ctx, to, responseFormat, count, usageJSON)
	return shinwayexecutor.Response{Payload: translatedUsage}, nil
}

// Refresh is a no-op for API-key based compatibility providers.
func (e *OpenAICompatExecutor) Refresh(ctx context.Context, auth *shinwayauth.Auth) (*shinwayauth.Auth, error) {
...
func openAICompatImageEndpointPath(opts shinwayexecutor.Options) string {
...
func (e *OpenAICompatExecutor) resolveCredentials(auth *shinwayauth.Auth) (baseURL, apiKey string) {
	if auth == nil {
		return "", ""
	}
...
func (e *OpenAICompatExecutor) resolveCompatConfig(auth *shinwayauth.Auth) *config.OpenAICompatibility {
