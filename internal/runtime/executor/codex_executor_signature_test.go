...
func newCodexSignatureTestAuth(serverURL string) *shinwayauth.Auth {
	return &shinwayauth.Auth{Attributes: map[string]string{
		"base_url": serverURL,
		"api_key":  "test",
	}}
}
...
func TestCodexExecutorDropsInvalidReasoningEncryptedContentFromFinalRequest(t *testing.T) {
	...
	_, err := executor.Execute(context.Background(), newCodexSignatureTestAuth(server.URL), shinwayexecutor.Request{
		Model: "gpt-5.4",
...
func TestCodexExecutorExecuteStreamDropsInvalidReasoningEncryptedContentFromFinalRequest(t *testing.T) {
	...
	executor := NewCodexExecutor(&config.Config{})
	result, err := executor.ExecuteStream(context.Background(), newCodexSignatureTestAuth(server.URL), shinwayexecutor.Request{
		Model:   "gpt-5.4",
