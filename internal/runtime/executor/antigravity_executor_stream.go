...
func (e *AntigravityExecutor) ExecuteStream(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (_ *shinwayexecutor.StreamResult, err error) {
	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		for idx, baseURL := range baseURLs {
			...
			go func(resp *http.Response) {
				...
				for i := range tail {
					select {
					case out <- shinwayexecutor.StreamChunk{Payload: tail[i]}:
					case <-ctx.Done():
						return
					}
				}
