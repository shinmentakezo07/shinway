...
func TestCodexExecutorTransportFailureBeforeTerminalIsRequestScoped(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			req := shinwayexecutor.Request{Model: "gpt-5.5", Payload: []byte(`{"model":"gpt-5.5","input":"hello"}`)}
			opts := shinwayexecutor.Options{SourceFormat: sdktranslator.FromString("openai-response"), Stream: tc.stream}
...
func TestCodexExecutorExecuteIgnoresTransportErrorAfterCompletion(t *testing.T) {
	...
	resp, err := executor.Execute(ctx, auth, shinwayexecutor.Request{
		Model:   "gpt-5.5",
		Payload: []byte(`{"model":"gpt-5.5","input":"hello"}`),
	}, shinwayexecutor.Options{
		SourceFormat: sdktranslator.FromString("openai-response"),
...
func TestCodexExecutorExecuteStreamIgnoresTransportErrorAfterCompletion(t *testing.T) {
	...
	result, err := executor.ExecuteStream(ctx, auth, shinwayexecutor.Request{
		Model:   "gpt-5.5",
		Payload: []byte(`{"model":"gpt-5.5","input":"hello"}`),
	}, shinwayexecutor.Options{
		SourceFormat: sdktranslator.FromString("openai-response"),
