...
type codexWebsocketSession struct {
	...
	lifecycle       shinwayexecutor.ExecutionLifecycle
...
func (s *codexWebsocketSession) bindExecutionLifecycle(opts shinwayexecutor.Options, conn *websocket.Conn, closer *websocketConnectionCloser, model string) error {
	if closer == nil {
		return fmt.Errorf("codex websockets executor: websocket connection closer is nil")
	}
	if s == nil {
		return shinwayexecutor.BindExecutionResource(opts, closer)
	}
...
func (s *codexWebsocketSession) closeBoundConnection(conn *websocket.Conn, closer *websocketConnectionCloser, lifecycle shinwayexecutor.ExecutionLifecycle) error {
	if s == nil || conn == nil {
		return nil
	}
...
func (s *codexWebsocketSession) detachConnection(conn *websocket.Conn, lifecycle shinwayexecutor.ExecutionLifecycle) *websocketConnectionCloser {
	if s == nil || conn == nil {
		return nil
	}
	s.connMu.Lock()
...
func detachMismatchedWebsocketSessionConn(sess *codexWebsocketSession, authID string, wsURL string) (*websocket.Conn, *websocketConnectionCloser, string, string, shinwayexecutor.ExecutionLifecycle) {
	if sess == nil {
		return nil, nil, "", "", nil
	}
...
func executionSessionIDFromOptions(opts shinwayexecutor.Options) string {
	if len(opts.Metadata) == 0 {
		return ""
	}
	raw, ok := opts.Metadata[shinwayexecutor.ExecutionSessionMetadataKey]
	if !ok || raw == nil {
		return ""
	}
	switch v := raw.(type) {
...
func (e *CodexWebsocketsExecutor) ensureUpstreamConn(ctx context.Context, auth *shinwayauth.Auth, sess *codexWebsocketSession, authID string, wsURL string, headers http.Header) (*websocket.Conn, *websocketConnectionCloser, *http.Response, error) {
	if sess == nil {
