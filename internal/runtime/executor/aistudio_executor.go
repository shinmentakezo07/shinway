...
func (e *AIStudioExecutor) PrepareRequest(req *http.Request, auth *shinwayauth.Auth) error {
	if req == nil {
		return nil
	}
...
func (e *AIStudioExecutor) HttpRequest(ctx context.Context, auth *shinwayauth.Auth, req *http.Request) (*http.Response, error) {
	if req == nil {
...
func (e *AIStudioExecutor) Execute(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (resp shinwayexecutor.Response, err error) {
	...
	resp = shinwayexecutor.Response{Payload: ensureColonSpacedJSON(out), Headers: wsResp.Headers.Clone()}
	return resp, nil
}

// ExecuteStream performs a streaming request to the AI Studio API.
func (e *AIStudioExecutor) ExecuteStream(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (_ *shinwayexecutor.StreamResult, err error) {
	if opts.Alt == "responses/compact" {
	...
	go func(first wsrelay.StreamEvent) {
		...
		processEvent := func(event wsrelay.StreamEvent) bool {
			if event.Err != nil {
				...
				select {
				case out <- shinwayexecutor.StreamChunk{Err: fmt.Errorf("wsrelay: %v", event.Err)}:
				case <-ctx.Done():
				}
				return false
			}
			...
			case wsrelay.MessageTypeStreamChunk:
				if len(event.Payload) > 0 {
					...
					for i := range lines {
						select {
						case out <- shinwayexecutor.StreamChunk{Payload: ensureColonSpacedJSON(lines[i])}:
						case <-ctx.Done():
							return false
						}
					}
	...
	return &shinwayexecutor.StreamResult{Headers: firstEvent.Headers.Clone(), Chunks: out}, nil
}

// CountTokens counts tokens for the given request using the AI Studio API.
func (e *AIStudioExecutor) CountTokens(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
	...
	if resp.Status < 200 || resp.Status >= 300 {
		return shinwayexecutor.Response{}, statusErr{code: resp.Status, msg: string(resp.Body)}
	}
	totalTokens := gjson.GetBytes(resp.Body, "totalTokens").Int()
	if totalTokens <= 0 {
		return shinwayexecutor.Response{}, fmt.Errorf("wsrelay: totalTokens missing in response")
	}
	responseFormat := shinwayexecutor.ResponseFormatOrSource(opts)
	translated := sdktranslator.TranslateTokenCount(ctx, body.toFormat, responseFormat, totalTokens, resp.Body)
	return shinwayexecutor.Response{Payload: translated}, nil
}

// Refresh refreshes the authentication credentials (no-op for AI Studio).
func (e *AIStudioExecutor) Refresh(ctx context.Context, auth *shinwayauth.Auth) (*shinwayauth.Auth, error) {
...
func (e *AIStudioExecutor) translateRequest(ctx context.Context, req shinwayexecutor.Request, opts shinwayexecutor.Options, stream bool) ([]byte, translatedPayload, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
