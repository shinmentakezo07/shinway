...
func (e *CodexWebsocketsExecutor) ExecuteStream(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (_ *shinwayexecutor.StreamResult, err error) {
	...
	if errDial != nil {
		...
		if respHS != nil && respHS.StatusCode == http.StatusUpgradeRequired {
			...
			if opts.ExecutionLifecycle != nil || shinwayexecutor.DownstreamWebsocket(ctx) {
		...
		for {
			...
			if msgType != websocket.TextMessage {
				if msgType == websocket.BinaryMessage {
					...
					_ = send(shinwayexecutor.StreamChunk{Err: err})
					return
				}
				continue
			}
			...
			if wsErr, ok := parseCodexWebsocketError(payload); ok {
				...
				if errClearReplay := clearCodexReasoningReplayOnWebsocketError(ctx, replayScope, payload); errClearReplay != nil {
					...
					_ = send(shinwayexecutor.StreamChunk{Err: errClearReplay})
					return
				}
			...
			if streamErr, terminalBody, ok := codexTerminalFailureErr(payload); ok {
				...
				_ = send(shinwayexecutor.StreamChunk{Err: streamErr})
				return
			}
			...
			if shinwayexecutor.DownstreamWebsocket(ctx) {
				if !send(shinwayexecutor.StreamChunk{Payload: clientPayload}) {
					terminateReason = "context_done"
					terminateErr = ctx.Err()
					return
				}
