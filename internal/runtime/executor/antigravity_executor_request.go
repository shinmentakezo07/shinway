...
func (e *AntigravityExecutor) buildRequest(ctx context.Context, auth *shinwayauth.Auth, token, modelName string, payload []byte, stream bool, alt, baseURL string, derivedSessionIDs ...string) (*http.Request, error) {
	if token == "" {
...
func buildBaseURL(auth *shinwayauth.Auth) string {
...
func antigravityLoadCodeAssistBaseURL(auth *shinwayauth.Auth) string {
...
func resolveUserAgent(auth *shinwayauth.Auth) string {
	return misc.AntigravityRequestUserAgent(antigravityConfiguredUserAgent(auth))
}

func resolveLoadCodeAssistUserAgent(auth *shinwayauth.Auth) string {
	return misc.AntigravityLoadCodeAssistUserAgent(antigravityConfiguredUserAgent(auth))
}

func antigravityConfiguredUserAgent(auth *shinwayauth.Auth) string {
	raw := ""
	if auth != nil {
...
var antigravityBaseURLFallbackOrder = func(auth *shinwayauth.Auth) []string {
	if base := resolveCustomAntigravityBaseURL(auth); base != "" {
		return []string{base}
	}
	return []string{
...
func resolveCustomAntigravityBaseURL(auth *shinwayauth.Auth) string {
	if auth == nil {
		return ""
	}
	if auth.Attributes != nil {
