...
func (e *XAIExecutor) CountTokens(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	...
	if err != nil {
		return shinwayexecutor.Response{}, fmt.Errorf("xai executor: tokenizer init failed: %w", err)
	}
	count, err := countXAIInputTokens(enc, prepared.body)
	if err != nil {
		return shinwayexecutor.Response{}, fmt.Errorf("xai executor: token counting failed: %w", err)
	}
