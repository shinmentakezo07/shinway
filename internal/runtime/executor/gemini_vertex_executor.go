...
import (
	...
	vertexauth "github.com/shinmentakezo07/shinway/v7/internal/auth/vertex"
...
func (e *GeminiVertexExecutor) PrepareRequest(req *http.Request, auth *shinwayauth.Auth) error {
	if req == nil {
		return nil
	}
...
func (e *GeminiVertexExecutor) HttpRequest(ctx context.Context, auth *shinwayauth.Auth, req *http.Request) (*http.Response, error) {
	if req == nil {
...
func (e *GeminiVertexExecutor) Execute(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (resp shinwayexecutor.Response, err error) {
...
func (e *GeminiVertexExecutor) ExecuteStream(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (*shinwayexecutor.StreamResult, error) {
	if opts.Alt == "responses/compact" {
...
func (e *GeminiVertexExecutor) CountTokens(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	// Try API key authentication first
	apiKey, baseURL := vertexAPICreds(auth)
	...
	if apiKey == "" {
		projectID, location, saJSON, errCreds := vertexCreds(auth)
		if errCreds != nil {
			return shinwayexecutor.Response{}, errCreds
		}
...
func (e *GeminiVertexExecutor) Refresh(ctx context.Context, auth *shinwayauth.Auth) (*shinwayauth.Auth, error) {
...
func (e *GeminiVertexExecutor) executeWithServiceAccount(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options, projectID, location string, saJSON []byte) (resp shinwayexecutor.Response, err error) {
	...
	resp = shinwayexecutor.Response{Payload: out, Headers: httpResp.Header.Clone()}
	return resp, nil
}

// executeWithAPIKey handles authentication using API key credentials.
func (e *GeminiVertexExecutor) executeWithAPIKey(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options, apiKey, baseURL string) (resp shinwayexecutor.Response, err error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
...
func (e *GeminiVertexExecutor) executeStreamWithServiceAccount(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options, projectID, location string, saJSON []byte) (_ *shinwayexecutor.StreamResult, err error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
	...
	go func() {
		...
		for scanner.Scan() {
			...
			for i := range lines {
				select {
				case out <- shinwayexecutor.StreamChunk{Payload: lines[i]}:
				case <-ctx.Done():
					return
				}
			}
		...
		if errScan := scanner.Err(); errScan != nil {
			...
			select {
			case out <- shinwayexecutor.StreamChunk{Err: errScan}:
			case <-ctx.Done():
			}
		}
	}()
	return &shinwayexecutor.StreamResult{Headers: httpResp.Header.Clone(), Chunks: out}, nil
}

// executeStreamWithAPIKey handles streaming authentication using API key credentials.
func (e *GeminiVertexExecutor) executeStreamWithAPIKey(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options, apiKey, baseURL string) (_ *shinwayexecutor.StreamResult, err error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
...
func (e *GeminiVertexExecutor) countTokensWithServiceAccount(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options, projectID, location string, saJSON []byte) (shinwayexecutor.Response, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
	...
	if errNewReq != nil {
		return shinwayexecutor.Response{}, errNewReq
	}
	httpReq.Header.Set("Content-Type", "application/json")
	if token, errTok := vertexAccessToken(ctx, e.cfg, auth, saJSON); errTok == nil && token != "" {
		httpReq.Header.Set("Authorization", "Bearer "+token)
		...
		log.Errorf("vertex executor: access token error: %v", errTok)
		return shinwayexecutor.Response{}, statusErr{code: 500, msg: "internal server error"}
	}
	applyGeminiHeaders(httpReq, auth)
	var attrs map[string]string
	if auth != nil {
	...
	if errDo != nil {
		helps.RecordAPIResponseError(ctx, e.cfg, errDo)
		return shinwayexecutor.Response{}, errDo
	}
	defer func() {
		if errClose := httpResp.Body.Close(); errClose != nil {
			log.Errorf("vertex executor: close response body error: %v", errClose)
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		helps.AppendAPIResponseChunk(ctx, e.cfg, b)
		helps.LogWithRequestID(ctx).Debugf("request error, error status: %d, error message: %s", httpResp.StatusCode, helps.SummarizeErrorBody(httpResp.Header.Get("Content-Type"), b))
		return shinwayexecutor.Response{}, statusErr{code: httpResp.StatusCode, msg: string(b)}
	}
	data, errRead := io.ReadAll(httpResp.Body)
	if errRead != nil {
		helps.RecordAPIResponseError(ctx, e.cfg, errRead)
		return shinwayexecutor.Response{}, errRead
	}
	helps.AppendAPIResponseChunk(ctx, e.cfg, data)
	count := gjson.GetBytes(data, "totalTokens").Int()
	out := sdktranslator.TranslateTokenCount(ctx, to, responseFormat, count, data)
	return shinwayexecutor.Response{Payload: out, Headers: httpResp.Header.Clone()}, nil
}

// countTokensWithAPIKey handles token counting using API key credentials.
func (e *GeminiVertexExecutor) countTokensWithAPIKey(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options, apiKey, baseURL string) (shinwayexecutor.Response, error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
...
func vertexCreds(a *shinwayauth.Auth) (projectID, location string, serviceAccountJSON []byte, err error) {
...
func vertexAPICreds(a *shinwayauth.Auth) (apiKey, baseURL string) {
	if a == nil {
		return "", ""
	}
	if a.Attributes != nil {
...
func vertexAccessToken(ctx context.Context, cfg *config.Config, auth *shinwayauth.Auth, saJSON []byte) (string, error) {
...
func (e *GeminiVertexExecutor) resolveVertexConfig(auth *shinwayauth.Auth) *config.VertexCompatKey {
