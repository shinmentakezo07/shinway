...
func TestCodexWebsocketsExecuteRestoresClaudeAgentReasoningReplay(t *testing.T) {
	...
	auth := &shinwayauth.Auth{Provider: "codex", Attributes: map[string]string{"api_key": "sk-test", "base_url": server.URL}}
	...
	opts := shinwayexecutor.Options{SourceFormat: sdktranslator.FromString("claude"), Headers: headers}
...
func TestCodexWebsocketsExecuteResponsesLiteDoesNotInjectImageGenerationTool(t *testing.T) {
	...
	opts := shinwayexecutor.Options{SourceFormat: sdktranslator.FromString("codex")}
...
func TestCodexWebsocketsExecutePreservesPreviousResponseIDUpstream(t *testing.T) {
	...
	exec := NewCodexWebsocketsExecutor(&config.Config{SDKConfig: config.SDKConfig{DisableImageGeneration: config.DisableImageGenerationAll}})
	auth := &shinwayauth.Auth{Attributes: map[string]string{"api_key": "sk-test", "base_url": server.URL}}
	req := shinwayexecutor.Request{
		Model:   "gpt-5-codex",
...
func TestCodexWebsocketsExecuteStreamUpgradeRequiredReturnsWithoutLockingSession(t *testing.T) {
	...
	execute := func(payload string) {
		...
		go func() {
			_, errExecute := exec.ExecuteStream(ctx, auth, shinwayexecutor.Request{
				Model:   "gpt-5.4",
...
func TestCodexWebsocketsExecuteStreamHandshakeErrorReturnsWithoutLockingSession(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		go func() {
			_, errExecute := exec.ExecuteStream(context.Background(), auth, shinwayexecutor.Request{
				Model:   "gpt-5.4",
...
func TestCodexAutoExecutorRequiredUpstreamWebsocketRejectsHTTPFallback(t *testing.T) {
	...
	ctx := shinwayexecutor.WithRequiredUpstreamWebsocket(
	...
	}, shinwayexecutor.Options{SourceFormat: sdktranslator.FromString("openai-response")})
	if errExecute == nil {
	...
	requestScoped, ok := errExecute.(shinwayexecutor.RequestScopedError)
...
func TestApplyCodexWebsocketHeadersPreservesExplicitAPIKeyUserAgent(t *testing.T) {
	auth := &shinwayauth.Auth{Provider: "codex", Attributes: map[string]string{"api_key": "sk-test"}}
...
func TestApplyCodexWebsocketHeadersUsesCanonicalAccountHeader(t *testing.T) {
	auth := &shinwayauth.Auth{Provider: "codex", Metadata: map[string]any{"account_id": "acct-1"}}
...
func TestApplyCodexPromptCacheHeadersSetsSessionIDAndLegacyConversation(t *testing.T) {
	req := shinwayexecutor.Request{Model: "gpt-5-codex", Payload: []byte(`{"prompt_cache_key":"cache-1"}`)}
...
func TestApplyCodexPromptCacheHeadersKeepsExecutionSessionAcrossIncrementalRoots(t *testing.T) {
	...
	firstReq := shinwayexecutor.Request{
		Model:   "gpt-5-codex",
		Payload: []byte(`{"input":"first"}`),
		Metadata: map[string]any{
			shinwayexecutor.ExecutionSessionMetadataKey: "connection-1",
			shinwayexecutor.DerivedSessionIDMetadataKey: "ctx:v1:first-root",
		},
	}
	secondReq := shinwayexecutor.Request{
		Model:   "gpt-5-codex",
		Payload: []byte(`{"input":"second"}`),
		Metadata: map[string]any{
			shinwayexecutor.ExecutionSessionMetadataKey: "connection-1",
			shinwayexecutor.DerivedSessionIDMetadataKey: "ctx:v1:second-root",
		},
	}
...
func TestApplyCodexWebsocketHeadersIdentityConfuseRemapsPromptCacheKey(t *testing.T) {
	...
	auth := &shinwayauth.Auth{ID: "auth-ws-1", Provider: "codex"}
	req := shinwayexecutor.Request{
		Model:   "gpt-5-codex",
...
func TestCodexWebsocketUpgradeRequiredDoesNotFallbackToHTTPWithLifecycle(t *testing.T) {
	...
	exec := NewCodexWebsocketsExecutor(&config.Config{SDKConfig: config.SDKConfig{DisableImageGeneration: config.DisableImageGenerationAll}})
	auth := &shinwayauth.Auth{ID: "auth-a", Provider: "codex", Attributes: map[string]string{"api_key": "sk-test", "base_url": server.URL}}
	...
	for _, statusCode := range []int{http.StatusUpgradeRequired, http.StatusBadGateway} {
		t.Run(http.StatusText(statusCode), func(t *testing.T) {
			...
			opts := shinwayexecutor.Options{
				...
				ResponseFormat: sdktranslator.FromString("openai-response"),
				Metadata: map[string]any{
					shinwayexecutor.ExecutionSessionMetadataKey: "failed-handshake",
				},
			}
...
func TestCodexWebsocketTerminalFailureInvalidatesRetainedLifecycle(t *testing.T) {
	...
	opts := shinwayexecutor.Options{
		...
		ExecutionLifecycle: newTerminalFailureLifecycle(),
		Metadata: map[string]any{
			shinwayexecutor.ExecutionSessionMetadataKey: "terminal-failure",
		},
	}
...
func TestCodexWebsocketNonstreamLifecycleBindFailureDetachesConnection(t *testing.T) {
	...
	opts := shinwayexecutor.Options{
		...
		Metadata: map[string]any{
			shinwayexecutor.ExecutionSessionMetadataKey: "nonstream-bind-failed",
		},
	}
	if _, errExecute := exec.Execute(context.Background(), auth, req, opts); errExecute == nil {
...
func TestCodexWebsocketLifecycleBindFailureReleasesSessionRequestLock(t *testing.T) {
	...
	opts := shinwayexecutor.Options{
		...
		ExecutionLifecycle: rejectingExecutionLifecycle{},
		Metadata: map[string]any{
			shinwayexecutor.ExecutionSessionMetadataKey: "bind-failed",
		},
	}
	if _, errExecute := exec.ExecuteStream(context.Background(), auth, req, opts); errExecute == nil {
