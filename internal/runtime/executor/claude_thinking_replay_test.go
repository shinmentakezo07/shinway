...
const claudeReplayResolvedModelInfoKey = "shinway.resolved_api_key_model_info"

func claudeReplayTestAuth(baseURL string) *shinwayauth.Auth {
	return &shinwayauth.Auth{
		ID:       "claude-replay-auth",
		Provider: "claude",
		Attributes: map[string]string{
			shinwayauth.AttributeAPIKey:   "key-claude-replay",
			shinwayauth.AttributeAuthKind: shinwayauth.AuthKindAPIKey,
			"base_url":                    baseURL,
		},
	}
}

func claudeReplayTestRequest(payload []byte, sessionID string, isCompat bool, source sdktranslator.Format) (shinwayexecutor.Request, shinwayexecutor.Options) {
	return shinwayexecutor.Request{
			...
			SourceFormat: source,
			Headers:      http.Header{"X-Shinway-Replay-Test": []string{"isolated"}},
			Metadata: map[string]any{
				shinwayexecutor.ExecutionSessionMetadataKey: sessionID,
			},
		}
}
...
func TestClaudeThinkingReplayEnabledRequiresCompatClaudeAPIKey(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			auth: baseAuth,
			request: func() shinwayexecutor.Request {
		...
		},
		{
			...
			auth: func() *shinwayauth.Auth {
				auth := baseAuth.Clone()
				auth.Attributes[shinwayauth.AttributeAuthKind] = shinwayauth.AuthKindOAuth
				auth.Attributes[shinwayauth.AttributeAPIKey] = "sk-ant-oat-replay"
				return auth
			}(),
			request:    baseRequest,
			options:    baseOptions,
		...
		},
		{
			...
			auth:    baseAuth,
			request: baseRequest,
			options: func() shinwayexecutor.Options {
				options := baseOptions
...
func TestClaudeExecutorCompatThinkingReplayRestoresOmittedBlock(t *testing.T) {
	...
	firstScope := claudeThinkingReplayScopeFromRequest(context.Background(), auth, firstRequest, firstOptions)
	if content, found, errGet := internalcache.GetClaudeThinkingReplayRequired(context.Background(), firstScope.modelFamily, firstScope.sessionKey); errGet != nil || !found || len(content) != 1 {
		t.Fatalf("first replay cache = (%s, %t, %v), want one cached assistant turn", content, found, errGet)
	}
