...
func TestKimiExecutorClaudeRequestPreservesInternalModelSemantics(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), "shinway.roundtripper", kimiRoundTripperFunc(func(req *http.Request) (*http.Response, error) {
		var errRead error
	...
	response, err := executor.Execute(ctx, auth, shinwayexecutor.Request{
		Model:   model,
		Payload: payload,
...
func TestKimiExecutorCountTokensUsesCanonicalUpstreamModel(t *testing.T) {
	...
	_, err := executor.CountTokens(ctx, auth, shinwayexecutor.Request{
...
func TestKimiExecutorClaudeStreamForwardsAnthropicBetaAndLogsUpstream(t *testing.T) {
	...
	ctx = context.WithValue(ctx, "shinway.roundtripper", kimiRoundTripperFunc(func(req *http.Request) (*http.Response, error) {
		upstreamRequest = req.Clone(req.Context())
