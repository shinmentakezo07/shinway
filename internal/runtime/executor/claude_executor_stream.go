...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (_ *shinwayexecutor.StreamResult, err error) {
	...
	go func() {
		...
		if responseFormat == to {
			...
			flushEvent := func() bool {
				...
				event.Reset()
				select {
				case out <- shinwayexecutor.StreamChunk{Payload: cloned}:
					return true
				case <-ctx.Done():
					return false
				}
	...
	return wrapClaudeThinkingReplayStream(ctx, &shinwayexecutor.StreamResult{Headers: httpResp.Header.Clone(), Chunks: out}, replayScope), nil
}
