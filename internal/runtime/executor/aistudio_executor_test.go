...
func TestAIStudioExecutorExecuteStartsTTFTBeforeRelayWait(t *testing.T) {
	...
	_, errExecute := exec.Execute(context.Background(), &shinwayauth.Auth{ID: authID, Provider: "aistudio"}, shinwayexecutor.Request{
		Model:   "gemini-3.1-pro-preview",
		Payload: []byte(`{"contents":[{"role":"user","parts":[{"text":"hi"}]}]}`),
	}, shinwayexecutor.Options{SourceFormat: sdktranslator.FormatGemini})
	if errExecute != nil {
