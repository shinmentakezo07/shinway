...
func (e *AntigravityExecutor) Execute(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (resp shinwayexecutor.Response, err error) {
	...
	useCredits := shinwayauth.AntigravityCreditsRequested(ctx) && antigravityCreditsRetryEnabled(e.cfg)
...
attemptLoop:
	for attempt := 0; attempt < attempts; attempt++ {
		...
		for idx, baseURL := range baseURLs {
			...
			resp = shinwayexecutor.Response{Payload: converted, Headers: httpResp.Header.Clone()}
...
func (e *AntigravityExecutor) executeClaudeNonStream(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (resp shinwayexecutor.Response, err error) {
	baseModel := thinking.ParseSuffix(req.Model).ModelName
	if inCooldown, remaining, errCooldown := antigravityIsInShortCooldownRequired(ctx, auth, baseModel, time.Now()); errCooldown != nil {
		return resp, homeKVUnavailableStatusErr(errCooldown)
	} else if inCooldown && !antigravityShouldBypassShortCooldown(ctx, e.cfg) {
		...
		for idx, baseURL := range baseURLs {
			...
			go func(resp *http.Response) {
				...
				for scanner.Scan() {
					...
					out <- shinwayexecutor.StreamChunk{Payload: payload}
				}
				if errScan := scanner.Err(); errScan != nil {
					helps.RecordAPIResponseError(ctx, e.cfg, errScan)
					reporter.PublishFailure(ctx, errScan)
					out <- shinwayexecutor.StreamChunk{Err: errScan}
				} else {
					if replayAccumulator != nil {
			...
			resp = shinwayexecutor.Response{Payload: e.convertStreamToNonStream(buffer.Bytes())}
