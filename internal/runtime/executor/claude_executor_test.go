...
func TestClaudeExecutor_CountTokensExcludesInvalidOpenAIThinking(t *testing.T) {
	...
	countTokens := func(payload []byte) int64 {
		t.Helper()
		resp, err := executor.CountTokens(context.Background(), nil, shinwayexecutor.Request{
			Model:   "claude-3-5-sonnet-20241022",
			Payload: payload,
		}, shinwayexecutor.Options{SourceFormat: sdktranslator.FromString("claude")})
		if err != nil {
...
func TestClaudeExecutor_CountTokensCountsLocallyWithoutUpstreamRequest(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			resp, errCount := executor.CountTokens(context.Background(), auth, shinwayexecutor.Request{
...
func TestClaudeExecutor_CountTokensRejectsInvalidRequests(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			_, err := executor.CountTokens(context.Background(), nil, shinwayexecutor.Request{
				Model:   "claude-sonnet-4-5",
				Payload: []byte(testCase.payload),
			}, shinwayexecutor.Options{SourceFormat: sdktranslator.FormatClaude})
			assertStatusErr(t, err, http.StatusBadRequest)
			requestErr, ok := err.(shinwayexecutor.RequestScopedError)
...
func TestClaudeExecutor_CountTokensRebuildsMidSystemMessagesBeforeValidation(t *testing.T) {
	...
	resp, err := executor.CountTokens(context.Background(), auth, shinwayexecutor.Request{
		Model:   "claude-sonnet-4-5",
		Payload: payload,
	}, shinwayexecutor.Options{SourceFormat: sdktranslator.FormatClaude})
	if err != nil {
...
func TestClaudeExecutor_ReusesUserIDAcrossModelsWhenCacheEnabled(t *testing.T) {
	...
	for _, model := range models {
		...
		if _, err := executor.Execute(context.Background(), auth, shinwayexecutor.Request{
			Model:   model,
...
func executeOpenAIChatCompletionThroughClaude(t *testing.T, upstreamBody string) (shinwayexecutor.Response, error) {
	t.Helper()
	...
	return executor.Execute(context.Background(), auth, shinwayexecutor.Request{
		Model:   "claude-3-5-sonnet-20241022",
		Payload: payload,
	}, shinwayexecutor.Options{
...
func TestClaudeExecutor_Execute_InvalidGzipErrorBodyReturnsDecodeMessage(t *testing.T) {
	testClaudeExecutorInvalidCompressedErrorBody(t, func(executor *ClaudeExecutor, auth *shinwayauth.Auth, payload []byte) error {
		_, err := executor.Execute(context.Background(), auth, shinwayexecutor.Request{
			Model:   "claude-3-5-sonnet-20241022",
			Payload: payload,
		}, shinwayexecutor.Options{SourceFormat: sdktranslator.FromString("claude")})
...
func testClaudeExecutorInvalidCompressedErrorBody(
	t *testing.T,
	invoke func(executor *ClaudeExecutor, auth *shinwayauth.Auth, payload []byte) error,
) {
	t.Helper()
...
func TestClaudeExecutor_RebuildMidSystemMessageDisabledByDefault(t *testing.T) {
	...
	_, errExecute := executor.Execute(ctx, auth, shinwayexecutor.Request{
		Model:   "claude-3-5-sonnet-20241022",
		Payload: payload,
	}, shinwayexecutor.Options{SourceFormat: sdktranslator.FromString("claude")})
	if errExecute != nil {
...
func TestApplyCloaking_PreservesConfiguredStrictModeAndSensitiveWordsWhenModeOmitted(t *testing.T) {
	...
	auth := &shinwayauth.Auth{Attributes: map[string]string{"api_key": "key-123"}}
...
func TestClaudeExecutor_ExecuteOpenAINonStreamRestoresOAuthToolNames(t *testing.T) {
	...
	resp, err := executor.Execute(context.Background(), auth, shinwayexecutor.Request{
		Model:   "claude-3-5-sonnet-20241022",
		Payload: payload,
	}, shinwayexecutor.Options{
		SourceFormat: sdktranslator.FromString("openai"),
