...
func antigravityAuthHasCredits(auth *shinwayauth.Auth) bool {
...
func antigravityAuthHasCreditsRequired(ctx context.Context, auth *shinwayauth.Auth) (bool, error) {
	if auth == nil || strings.TrimSpace(auth.ID) == "" {
		return false, nil
	}
	authID := strings.TrimSpace(auth.ID)
	if hint, ok, errHint := shinwayauth.GetAntigravityCreditsHintRequired(ctx, authID); errHint != nil {
...
func antigravityCreditsBalanceAvailable(authID string, bal antigravityCreditsBalance) bool {
	...
	shinwayauth.SetAntigravityCreditsHint(strings.TrimSpace(authID), shinwayauth.AntigravityCreditsHint{
		Known:           true,
		Available:       available,
...
func clearAntigravityCreditsFailureState(auth *shinwayauth.Auth) {
	if auth == nil || strings.TrimSpace(auth.ID) == "" {
		return
	}
	antigravityCreditsFailureByAuth.Delete(strings.TrimSpace(auth.ID))
}
func markAntigravityCreditsPermanentlyDisabled(auth *shinwayauth.Auth) {
	if auth == nil || strings.TrimSpace(auth.ID) == "" {
		return
	}
	authID := strings.TrimSpace(auth.ID)
	...
	shinwayauth.SetAntigravityCreditsHint(authID, shinwayauth.AntigravityCreditsHint{
		Known:           true,
		Available:       false,
		CreditAmount:    0,
		MinCreditAmount: 1,
...
func clearAntigravityCreditsPermanentlyDisabled(auth *shinwayauth.Auth) {
	if auth == nil || strings.TrimSpace(auth.ID) == "" {
		return
	}
	antigravityCreditsFailureByAuth.Delete(strings.TrimSpace(auth.ID))
...
func (e *AntigravityExecutor) maybeRefreshAntigravityCreditsHint(ctx context.Context, auth *shinwayauth.Auth, accessToken string) {
	...
	if hint, ok := shinwayauth.GetAntigravityCreditsHint(authID); ok && hint.Known {
		return
	}
	...
	if client, homeMode, errClient := currentAntigravityKVClient(); homeMode {
		...
		if ctx != nil {
			if rt, ok := ctx.Value("shinway.roundtripper").(http.RoundTripper); ok && rt != nil {
				refreshCtx = context.WithValue(refreshCtx, "shinway.roundtripper", rt)
			}
		}
		refreshCtx, cancel := context.WithTimeout(refreshCtx, antigravityCreditsHintRefreshTimeout)
		authCopy := auth.Clone()
		go func(auth *shinwayauth.Auth, token string) {
			defer cancel()
	...
	go func(state *antigravityCreditsHintRefreshState, auth *shinwayauth.Auth, token string) {
		defer cancel()
...
func (e *AntigravityExecutor) updateAntigravityCreditsBalance(ctx context.Context, auth *shinwayauth.Auth, accessToken string) {
	if auth == nil || strings.TrimSpace(auth.ID) == "" {
		return
	}
...
func antigravityRetryAttempts(auth *shinwayauth.Auth, cfg *config.Config) int {
	retry := 0
	if cfg != nil {
...
func antigravityShouldBypassShortCooldown(ctx context.Context, cfg *config.Config) bool {
	return shinwayauth.AntigravityCreditsRequested(ctx) && antigravityCreditsRetryEnabled(cfg)
}
...
func antigravityShortCooldownKey(auth *shinwayauth.Auth, modelName string) string {
	if auth == nil {
		return ""
	}
	authID := strings.TrimSpace(auth.ID)
...
func antigravityShortCooldownKVKey(auth *shinwayauth.Auth, modelName string) string {
	if auth == nil {
		return ""
	}
	authID := strings.TrimSpace(auth.ID)
...
func antigravityIsInShortCooldown(auth *shinwayauth.Auth, modelName string, now time.Time) (bool, time.Duration) {
...
func antigravityIsInShortCooldownRequired(ctx context.Context, auth *shinwayauth.Auth, modelName string, now time.Time) (bool, time.Duration, error) {
...
func markAntigravityShortCooldown(auth *shinwayauth.Auth, modelName string, now time.Time, duration time.Duration) {
...
func markAntigravityShortCooldownRequired(ctx context.Context, auth *shinwayauth.Auth, modelName string, now time.Time, duration time.Duration) error {
	kvKey := antigravityShortCooldownKVKey(auth, modelName)
	client, homeMode, errClient := currentAntigravityKVClient()
	if homeMode {
		if errClient != nil {
