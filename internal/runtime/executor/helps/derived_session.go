...
func DerivedSessionID(metadataSets ...map[string]any) string {
	for _, metadata := range metadataSets {
		if derivedID := shinwaysession.DerivedID(metadata); derivedID != "" {
			return derivedID
		}
	}
	return ""
...
func ProviderSessionUUID(provider string, metadataSets ...map[string]any) string {
	for _, metadata := range metadataSets {
		if executionID := metadataString(metadata, shinwayexecutor.ExecutionSessionMetadataKey); executionID != "" {
