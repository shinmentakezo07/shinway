...
func TestResolveClaudeDeviceProfileRequiredHomeReadWithoutCandidate(t *testing.T) {
	...
	auth := &shinwayauth.Auth{ID: "auth-1"}
...
func TestResolveClaudeDeviceProfileRequiredHomeFailures(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, errProfile := ResolveClaudeDeviceProfileRequired(context.Background(), &shinwayauth.Auth{ID: "auth-1"}, "api-key", tc.headers, nil); errProfile == nil {
