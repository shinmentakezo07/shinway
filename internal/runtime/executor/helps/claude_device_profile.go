...
func claudeDeviceProfileScopeKey(auth *shinwayauth.Auth, apiKey string) string {
	switch {
...
func claudeDeviceProfileCacheKey(auth *shinwayauth.Auth, apiKey string) string {
	sum := sha256.Sum256([]byte(claudeDeviceProfileScopeKey(auth, apiKey)))
	return hex.EncodeToString(sum[:])
}

func claudeDeviceProfileKVKey(auth *shinwayauth.Auth, apiKey string) string {
	return "cpa:claude:device-profile:" + homekv.HashKeyPart(claudeDeviceProfileScopeKey(auth, apiKey))
}

func claudeDeviceProfileLockKVKey(auth *shinwayauth.Auth, apiKey string) string {
...
func ResolveClaudeDeviceProfile(auth *shinwayauth.Auth, apiKey string, headers http.Header, cfg *config.Config) ClaudeDeviceProfile {
...
func ResolveClaudeDeviceProfileRequired(ctx context.Context, auth *shinwayauth.Auth, apiKey string, headers http.Header, cfg *config.Config) (ClaudeDeviceProfile, error) {
...
func resolveClaudeDeviceProfileLocal(auth *shinwayauth.Auth, apiKey string, headers http.Header, cfg *config.Config) ClaudeDeviceProfile {
...
func resolveClaudeDeviceProfileHome(ctx context.Context, client claudeDeviceProfileKVClient, auth *shinwayauth.Auth, apiKey string, headers http.Header, cfg *config.Config) (ClaudeDeviceProfile, error) {
	baseline := defaultClaudeDeviceProfile(cfg)
	candidate, hasCandidate := extractClaudeDeviceProfile(headers, cfg)
	if hasCandidate {
		candidate = pinClaudeDeviceProfilePlatform(candidate, baseline)
