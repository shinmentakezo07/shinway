...
func NewExecutorUsageReporter(ctx context.Context, executor usageExecutor, model string, auth *shinwayauth.Auth) *UsageReporter {
	provider := ""
	if executor != nil {
...
func NewUsageReporter(ctx context.Context, provider, model string, auth *shinwayauth.Auth) *UsageReporter {
...
func resolveUsageSource(auth *shinwayauth.Auth, ctxAPIKey string) string {
	if auth != nil {
...
func resolveUsageAuthType(auth *shinwayauth.Auth) string {
	if auth == nil {
		return ""
	}
