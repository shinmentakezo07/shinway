...
func PayloadRequestedModel(opts shinwayexecutor.Options, fallback string) string {
...
func PayloadRequestPath(opts shinwayexecutor.Options) string {
	if len(opts.Metadata) == 0 {
		return ""
	}
	raw, ok := opts.Metadata[shinwayexecutor.RequestPathMetadataKey]
	if !ok || raw == nil {
		return ""
	}
	switch v := raw.(type) {
