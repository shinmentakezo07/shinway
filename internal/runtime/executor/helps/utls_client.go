...
func NewUtlsHTTPClient(ctx context.Context, cfg *config.Config, auth *shinwayauth.Auth, timeout time.Duration) *http.Client {
	var proxyURL string
	if auth != nil {
	...
	if ctx != nil {
		ctxRoundTripper, _ = ctx.Value("shinway.roundtripper").(http.RoundTripper)
	}
