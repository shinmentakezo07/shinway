...
func (e *XAIExecutor) executeImages(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, endpointPath string) (resp shinwayexecutor.Response, err error) {
...
func (e *XAIExecutor) executeVideos(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (resp shinwayexecutor.Response, err error) {
	model := strings.TrimSpace(gjson.GetBytes(req.Payload, "model").String())
	if model == "" {
		model = strings.TrimSpace(req.Model)
	}
