...
import (
	...
	kimiauth "github.com/shinmentakezo07/shinway/v7/internal/auth/kimi"
	"github.com/shinmentakezo07/shinway/v7/internal/buildinfo"
...
func (e *KimiExecutor) PrepareRequest(req *http.Request, auth *shinwayauth.Auth) error {
	if req == nil {
		return nil
	}
...
func (e *KimiExecutor) HttpRequest(ctx context.Context, auth *shinwayauth.Auth, req *http.Request) (*http.Response, error) {
	if req == nil {
...
func (e *KimiExecutor) ExecuteStream(ctx context.Context, auth *shinwayauth.Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (_ *shinwayexecutor.StreamResult, err error) {
	...
	go func() {
		...
		for i := range doneChunks {
			select {
			case out <- shinwayexecutor.StreamChunk{Payload: doneChunks[i]}:
			case <-ctx.Done():
				return
			}
		}
...
func (e *KimiExecutor) Refresh(ctx context.Context, auth *shinwayauth.Auth) (*shinwayauth.Auth, error) {
...
// Headers identify shinway with the current build version.
func applyKimiHeaders(r *http.Request, token string, stream bool) {
	r.Header.Set("Content-Type", "application/json")
	r.Header.Set("Authorization", "Bearer "+token)
	// Identify requests with the current shinway version.
	r.Header.Set("User-Agent", "shinway/"+buildinfo.Version)
	r.Header.Set("X-Msh-Platform", "shinway")
...
func resolveKimiDeviceIDFromAuth(auth *shinwayauth.Auth) string {
...
func resolveKimiDeviceIDFromStorage(auth *shinwayauth.Auth) string {
	if auth == nil {
		return ""
	}
...
func resolveKimiDeviceID(auth *shinwayauth.Auth) string {
...
func applyKimiHeadersWithAuth(r *http.Request, token string, stream bool, auth *shinwayauth.Auth) {
...
func kimiCreds(a *shinwayauth.Auth) (token string) {
	if a == nil {
		return ""
	}
...
// It strips the shinway "kimi-" prefix and any Claude Code "[1m]" context
