...
func applyXAIReasoningReplayCacheRequired(ctx context.Context, from sdktranslator.Format, req shinwayexecutor.Request, opts shinwayexecutor.Options, body []byte) ([]byte, xaiReasoningReplayScope, error) {
...
func xaiReasoningReplayScopeFromRequest(ctx context.Context, from sdktranslator.Format, req shinwayexecutor.Request, opts shinwayexecutor.Options, body []byte) xaiReasoningReplayScope {
	...
	if shinwayexecutor.DownstreamWebsocket(ctx) && strings.TrimSpace(gjson.GetBytes(req.Payload, "previous_response_id").String()) != "" {
		return xaiReasoningReplayScope{}
	}
