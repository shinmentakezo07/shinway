...
func resolveClaudeKeyConfig(cfg *config.Config, auth *shinwayauth.Auth) *config.ClaudeKey {
	if cfg == nil || auth == nil {
		return nil
	}
...
func resolveClaudeKeyCloakConfig(cfg *config.Config, auth *shinwayauth.Auth) *config.CloakConfig {
...
func experimentalCCHSigningEnabled(cfg *config.Config, auth *shinwayauth.Auth) bool {
	entry := resolveClaudeKeyConfig(cfg, auth)
	return entry != nil && entry.ExperimentalCCHSigning
}

func rebuildMidSystemMessageEnabled(cfg *config.Config, auth *shinwayauth.Auth) bool {
