...
func TestExecuteCommandLinePassesAllArgsAndTriggeredFlags(t *testing.T) {
	...
	exitCode, handled := host.ExecuteCommandLine(context.Background(), "shinway", []string{"-plugin-command", "tail"}, "/tmp/config.yaml", flagSet)
	if !handled {
	...
	if req.Program != "shinway" || req.ConfigPath != "/tmp/config.yaml" {
...
func TestExecuteCommandLinePersistsReturnedAuths(t *testing.T) {
	...
	exitCode, handled := host.ExecuteCommandLine(context.Background(), "shinway", []string{"-plugin-login"}, "/tmp/config.yaml", flagSet)
	if !handled {
		t.Fatal("ExecuteCommandLine() handled = false, want true")
	}
	if exitCode != 0 {
