...
const (
	shadowPluginPrefix           = "shinway-plugin-"
	shadowPluginTempPrefix       = ".shinway-plugin-"
...
func (dynamicLibraryLoader) Open(file pluginFile, host *Host) (pluginClient, error) {
	...
	proc, errProc := dll.FindProc("shinway_plugin_init")
	if errProc != nil {
		_ = dll.Release()
		removeShadowPlugin(loadPath)
		return nil, errProc
	...
	*hostCtx = id
	...
	if rc != 0 {
		...
		return nil, fmt.Errorf("shinway_plugin_init returned %d: %v", rc, errCall)
	}
...
func shadowPluginDir() (string, error) {
	dir := filepath.Join(os.TempDir(), "shinway-pluginhost", shadowPluginProcessDirName(os.Getpid()))
