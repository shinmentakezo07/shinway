...
import (
	...
	"github.com/shinmentakezo07/shinway/v7/internal/pluginstore"
)
...
func TestListPluginStoreEscapesRegistryStrings(t *testing.T) {
	...
	if entry.Name != html.EscapeString("<script>alert(1)</script>") ||
		...
		entry.Version != "0.1.0" ||
		entry.Repository != "https://github.com/author-name/shinway-sample-provider-plugin" ||
...
func TestListPluginStoreShowsLatestReleaseVersionAndCaches(t *testing.T) {
	...
	releaseCalls := httpClient.count("https://api.github.com/repos/author-name/shinway-sample-provider-plugin/releases/latest")
	if releaseCalls != 1 {
		...
		pluginStoreHTTPClient: fakePluginStoreHTTPClient{
			...
			"https://community.example/registry.json": []byte(`{
				...
				"plugins": [{
					...
					"author": "community",
					"version": "0.3.0",
					"repository": "https://github.com/community/shinway-third-provider-plugin"
				}]
			}`),
		},
	}
...
func TestInstallPluginFromStoreUsesRequestedThirdPartySource(t *testing.T) {
	...
	h := &Handler{
		...
		pluginStoreHTTPClient: fakePluginStoreHTTPClient{
			...
			"https://api.github.com/repos/community/shinway-sample-provider-plugin/releases/latest": []byte(`{
				"tag_name": "v0.3.0",
				"assets": [
					{"name": "` + archiveName + `", "browser_download_url": "https://downloads.example/` + archiveName + `"},
					{"name": "checksums.txt", "browser_download_url": "https://downloads.example/checksums.txt"}
...
func thirdPartySampleRegistryJSON(t *testing.T) []byte {
	...
	return []byte(`{
		...
		"plugins": [{
			...
			"author": "community",
			"version": "0.3.0",
			"repository": "https://github.com/community/shinway-sample-provider-plugin"
		}]
	}`)
}
