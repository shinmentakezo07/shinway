...
// nvidia-api-key: []NVIDIAKey
func (h *Handler) GetNVIDIAKeys(c *gin.Context) {
	c.JSON(200, gin.H{"nvidia-api-key": h.nvidiaKeysWithAuthIndex()})
}

func (h *Handler) PutNVIDIAKeys(c *gin.Context) {
	data, errRead := c.GetRawData()
	if errRead != nil {
		c.JSON(400, gin.H{"error": "failed to read body"})
		return
	}
	var arr []config.NVIDIAKey
	if errUnmarshal := json.Unmarshal(data, &arr); errUnmarshal != nil {
		var obj struct {
			Items []config.NVIDIAKey `json:"items"`
		}
		if errObject := json.Unmarshal(data, &obj); errObject != nil || len(obj.Items) == 0 {
			c.JSON(400, gin.H{"error": "invalid body"})
			return
	...
	filtered := make([]config.NVIDIAKey, 0, len(arr))
	for i := range arr {
		entry := arr[i]
		normalizeCodexKey(&entry)
		if entry.BaseURL == "" {
			entry.BaseURL = config.NVIDIAProviderBaseURL
		}
		if rejectInvalidCredentialWeight(c, fmt.Sprintf("nvidia-api-key[%d].weight", i), entry.Weight) {
			return
		}
		filtered = append(filtered, entry)
	}
	h.mu.Lock()
	defer h.mu.Unlock()
	h.cfg.NVIDIAKey = filtered
	h.cfg.SanitizeNVIDIAKeys()
	h.persistLocked(c)
}

func (h *Handler) PatchNVIDIAKey(c *gin.Context) {
	type nvidiaKeyPatch struct {
		APIKey         *string               `json:"api-key"`
		Priority       *int                  `json:"priority"`
		Weight         json.RawMessage       `json:"weight"`
		Prefix         *string               `json:"prefix"`
		...
		Websockets     *bool                 `json:"websockets"`
		ProxyURL       *string               `json:"proxy-url"`
		Models         *[]config.NVIDIAModel `json:"models"`
		Headers        *map[string]string    `json:"headers"`
		ExcludedModels *[]string             `json:"excluded-models"`
		DisableCooling *bool                 `json:"disable-cooling"`
	}
	var body struct {
		Index *int            `json:"index"`
		Match *string         `json:"match"`
		Value *nvidiaKeyPatch `json:"value"`
	}
	if errBind := c.ShouldBindJSON(&body); errBind != nil || body.Value == nil {
		c.JSON(400, gin.H{"error": "invalid body"})
		return
	...
	defer h.mu.Unlock()
	targetIndex := -1
	if body.Index != nil && *body.Index >= 0 && *body.Index < len(h.cfg.NVIDIAKey) {
		targetIndex = *body.Index
	}
	if targetIndex == -1 && body.Match != nil {
		match := strings.TrimSpace(*body.Match)
		for i := range h.cfg.NVIDIAKey {
			if h.cfg.NVIDIAKey[i].APIKey == match {
				targetIndex = i
				break
			}
		}
	...
	entry := h.cfg.NVIDIAKey[targetIndex]
	if body.Value.APIKey != nil {
		entry.APIKey = strings.TrimSpace(*body.Value.APIKey)
	}
	if body.Value.Priority != nil {
	...
	if body.Value.Models != nil {
		entry.Models = append([]config.NVIDIAModel(nil), (*body.Value.Models)...)
	}
	if body.Value.Headers != nil {
		entry.Headers = config.NormalizeHeaders(*body.Value.Headers)
	}
	...
	normalizeCodexKey(&entry)
	h.cfg.NVIDIAKey[targetIndex] = entry
	h.cfg.SanitizeNVIDIAKeys()
	h.persistLocked(c)
}

func (h *Handler) DeleteNVIDIAKey(c *gin.Context) {
	h.mu.Lock()
	defer h.mu.Unlock()
	if val := strings.TrimSpace(c.Query("api-key")); val != "" {
		if baseRaw, okBase := c.GetQuery("base-url"); okBase {
			base := strings.TrimSpace(baseRaw)
			out := make([]config.NVIDIAKey, 0, len(h.cfg.NVIDIAKey))
			for _, entry := range h.cfg.NVIDIAKey {
				if strings.TrimSpace(entry.APIKey) == val && strings.TrimSpace(entry.BaseURL) == base {
					continue
				}
				out = append(out, entry)
		...
		matchCount := 0
		for i := range h.cfg.NVIDIAKey {
			if strings.TrimSpace(h.cfg.NVIDIAKey[i].APIKey) == val {
				matchCount++
				if matchIndex == -1 {
					matchIndex = i
				}
		...
		if matchIndex != -1 {
			h.cfg.NVIDIAKey = append(h.cfg.NVIDIAKey[:matchIndex], h.cfg.NVIDIAKey[matchIndex+1:]...)
		}
		h.cfg.SanitizeNVIDIAKeys()
		h.persistLocked(c)
		return
	...
	if idxStr := c.Query("index"); idxStr != "" {
		var idx int
		_, errScan := fmt.Sscanf(idxStr, "%d", &idx)
		if errScan == nil && idx >= 0 && idx < len(h.cfg.NVIDIAKey) {
			h.cfg.NVIDIAKey = append(h.cfg.NVIDIAKey[:idx], h.cfg.NVIDIAKey[idx+1:]...)
			h.cfg.SanitizeNVIDIAKeys()
			h.persistLocked(c)
			return
		}
...
// zen-api-key: []ZenKey
func (h *Handler) GetZenKeys(c *gin.Context) {
	c.JSON(200, gin.H{"zen-api-key": h.zenKeysWithAuthIndex()})
}
func (h *Handler) PutZenKeys(c *gin.Context) {
	data, errRead := c.GetRawData()
	if errRead != nil {
		c.JSON(400, gin.H{"error": "failed to read body"})
		return
	}
	var arr []config.ZenKey
	if errUnmarshal := json.Unmarshal(data, &arr); errUnmarshal != nil {
		var obj struct {
			Items []config.ZenKey `json:"items"`
		}
		if errObject := json.Unmarshal(data, &obj); errObject != nil || len(obj.Items) == 0 {
			c.JSON(400, gin.H{"error": "invalid body"})
			return
	...
	filtered := make([]config.ZenKey, 0, len(arr))
	for i := range arr {
		entry := arr[i]
		normalizeCodexKey(&entry)
		if entry.BaseURL == "" {
			entry.BaseURL = config.ZenProviderBaseURL
		}
		if rejectInvalidCredentialWeight(c, fmt.Sprintf("zen-api-key[%d].weight", i), entry.Weight) {
			return
		}
		filtered = append(filtered, entry)
	}
	h.mu.Lock()
	defer h.mu.Unlock()
	h.cfg.ZenKey = filtered
	h.cfg.SanitizeZenKeys()
	h.persistLocked(c)
}
func (h *Handler) PatchZenKey(c *gin.Context) {
	...
	defer h.mu.Unlock()
	targetIndex := -1
	if body.Index != nil && *body.Index >= 0 && *body.Index < len(h.cfg.ZenKey) {
		targetIndex = *body.Index
	}
	if targetIndex == -1 && body.Match != nil {
		match := strings.TrimSpace(*body.Match)
		for i := range h.cfg.ZenKey {
			if h.cfg.ZenKey[i].APIKey == match {
				targetIndex = i
				break
			}
		}
	...
	entry := h.cfg.ZenKey[targetIndex]
	if body.Value.APIKey != nil {
		entry.APIKey = strings.TrimSpace(*body.Value.APIKey)
	}
	if body.Value.Priority != nil {
	...
	if body.Value.Models != nil {
		entry.Models = append([]config.ZenModel(nil), (*body.Value.Models)...)
	}
	if body.Value.Headers != nil {
		entry.Headers = config.NormalizeHeaders(*body.Value.Headers)
	}
	...
	normalizeCodexKey(&entry)
	h.cfg.ZenKey[targetIndex] = entry
	h.cfg.SanitizeZenKeys()
	h.persistLocked(c)
}

func (h *Handler) DeleteZenKey(c *gin.Context) {
	h.mu.Lock()
	defer h.mu.Unlock()
	if val := strings.TrimSpace(c.Query("api-key")); val != "" {
		if baseRaw, okBase := c.GetQuery("base-url"); okBase {
			base := strings.TrimSpace(baseRaw)
			out := make([]config.ZenKey, 0, len(h.cfg.ZenKey))
			for _, entry := range h.cfg.ZenKey {
				if strings.TrimSpace(entry.APIKey) == val && strings.TrimSpace(entry.BaseURL) == base {
					continue
				}
				out = append(out, entry)
		...
		matchCount := 0
		for i := range h.cfg.ZenKey {
			if strings.TrimSpace(h.cfg.ZenKey[i].APIKey) == val {
				matchCount++
				if matchIndex == -1 {
					matchIndex = i
				}
		...
		if matchIndex != -1 {
			h.cfg.ZenKey = append(h.cfg.ZenKey[:matchIndex], h.cfg.ZenKey[matchIndex+1:]...)
		}
		h.cfg.SanitizeZenKeys()
		h.persistLocked(c)
		return
	...
	if idxStr := c.Query("index"); idxStr != "" {
		var idx int
		_, errScan := fmt.Sscanf(idxStr, "%d", &idx)
		if errScan == nil && idx >= 0 && idx < len(h.cfg.ZenKey) {
			h.cfg.ZenKey = append(h.cfg.ZenKey[:idx], h.cfg.ZenKey[idx+1:]...)
			h.cfg.SanitizeZenKeys()
			h.persistLocked(c)
			return
		}
...
// tokenrouter-api-key: []TokenRouterKey
func (h *Handler) GetTokenRouterKeys(c *gin.Context) {
	c.JSON(200, gin.H{"tokenrouter-api-key": h.tokenRouterKeysWithAuthIndex()})
}
func (h *Handler) PutTokenRouterKeys(c *gin.Context) {
	data, errRead := c.GetRawData()
	if errRead != nil {
		c.JSON(400, gin.H{"error": "failed to read body"})
		return
	}
	var arr []config.TokenRouterKey
	if errUnmarshal := json.Unmarshal(data, &arr); errUnmarshal != nil {
		var obj struct {
			Items []config.TokenRouterKey `json:"items"`
		}
		if errObject := json.Unmarshal(data, &obj); errObject != nil || len(obj.Items) == 0 {
			c.JSON(400, gin.H{"error": "invalid body"})
			return
	...
	filtered := make([]config.TokenRouterKey, 0, len(arr))
	for i := range arr {
		entry := arr[i]
		normalizeCodexKey(&entry)
		if entry.BaseURL == "" {
			entry.BaseURL = config.TokenRouterProviderBaseURL
		}
		if rejectInvalidCredentialWeight(c, fmt.Sprintf("tokenrouter-api-key[%d].weight", i), entry.Weight) {
			return
		}
		filtered = append(filtered, entry)
	}
	h.mu.Lock()
	defer h.mu.Unlock()
	h.cfg.TokenRouterKey = filtered
	h.cfg.SanitizeTokenRouterKeys()
	h.persistLocked(c)
}
func (h *Handler) PatchTokenRouterKey(c *gin.Context) {
	type tokenRouterKeyPatch struct {
		APIKey         *string                    `json:"api-key"`
		Priority       *int                       `json:"priority"`
		Weight         json.RawMessage            `json:"weight"`
		Prefix         *string                    `json:"prefix"`
		...
		Websockets     *bool                      `json:"websockets"`
		ProxyURL       *string                    `json:"proxy-url"`
		Models         *[]config.TokenRouterModel `json:"models"`
		Headers        *map[string]string         `json:"headers"`
		ExcludedModels *[]string                  `json:"excluded-models"`
		DisableCooling *bool                      `json:"disable-cooling"`
	}
	var body struct {
		Index *int                 `json:"index"`
		Match *string              `json:"match"`
		Value *tokenRouterKeyPatch `json:"value"`
	}
	if errBind := c.ShouldBindJSON(&body); errBind != nil || body.Value == nil {
		c.JSON(400, gin.H{"error": "invalid body"})
		return
	...
	defer h.mu.Unlock()
	targetIndex := -1
	if body.Index != nil && *body.Index >= 0 && *body.Index < len(h.cfg.TokenRouterKey) {
		targetIndex = *body.Index
	}
	if targetIndex == -1 && body.Match != nil {
		match := strings.TrimSpace(*body.Match)
		for i := range h.cfg.TokenRouterKey {
			if h.cfg.TokenRouterKey[i].APIKey == match {
				targetIndex = i
				break
			}
		}
	...
	entry := h.cfg.TokenRouterKey[targetIndex]
	if body.Value.APIKey != nil {
		entry.APIKey = strings.TrimSpace(*body.Value.APIKey)
	}
	if body.Value.Priority != nil {
	...
	if body.Value.Models != nil {
		entry.Models = append([]config.TokenRouterModel(nil), (*body.Value.Models)...)
	}
	if body.Value.Headers != nil {
		entry.Headers = config.NormalizeHeaders(*body.Value.Headers)
	}
	...
	normalizeCodexKey(&entry)
	h.cfg.TokenRouterKey[targetIndex] = entry
	h.cfg.SanitizeTokenRouterKeys()
	h.persistLocked(c)
}

func (h *Handler) DeleteTokenRouterKey(c *gin.Context) {
	h.mu.Lock()
	defer h.mu.Unlock()
	if val := strings.TrimSpace(c.Query("api-key")); val != "" {
		if baseRaw, okBase := c.GetQuery("base-url"); okBase {
			base := strings.TrimSpace(baseRaw)
			out := make([]config.TokenRouterKey, 0, len(h.cfg.TokenRouterKey))
			for _, entry := range h.cfg.TokenRouterKey {
				if strings.TrimSpace(entry.APIKey) == val && strings.TrimSpace(entry.BaseURL) == base {
					continue
				}
				out = append(out, entry)
			}
			h.cfg.TokenRouterKey = out
			h.cfg.SanitizeTokenRouterKeys()
			h.persistLocked(c)
			return
		}
		...
		matchCount := 0
		for i := range h.cfg.TokenRouterKey {
			if strings.TrimSpace(h.cfg.TokenRouterKey[i].APIKey) == val {
				matchCount++
				if matchIndex == -1 {
					matchIndex = i
				}
		...
		if matchIndex != -1 {
			h.cfg.TokenRouterKey = append(h.cfg.TokenRouterKey[:matchIndex], h.cfg.TokenRouterKey[matchIndex+1:]...)
		}
		h.cfg.SanitizeTokenRouterKeys()
		h.persistLocked(c)
		return
	...
	if idxStr := c.Query("index"); idxStr != "" {
		var idx int
		_, errScan := fmt.Sscanf(idxStr, "%d", &idx)
		if errScan == nil && idx >= 0 && idx < len(h.cfg.TokenRouterKey) {
			h.cfg.TokenRouterKey = append(h.cfg.TokenRouterKey[:idx], h.cfg.TokenRouterKey[idx+1:]...)
			h.cfg.SanitizeTokenRouterKeys()
			h.persistLocked(c)
			return
		}
...
// orcarouter-api-key: []OrcaRouterKey
func (h *Handler) GetOrcaRouterKeys(c *gin.Context) {
	c.JSON(200, gin.H{"orcarouter-api-key": h.orcaRouterKeysWithAuthIndex()})
}
func (h *Handler) PutOrcaRouterKeys(c *gin.Context) {
	data, errRead := c.GetRawData()
	if errRead != nil {
		c.JSON(400, gin.H{"error": "failed to read body"})
		return
	}
	var arr []config.OrcaRouterKey
	if errUnmarshal := json.Unmarshal(data, &arr); errUnmarshal != nil {
		var obj struct {
			Items []config.OrcaRouterKey `json:"items"`
		}
		if errObject := json.Unmarshal(data, &obj); errObject != nil || len(obj.Items) == 0 {
			c.JSON(400, gin.H{"error": "invalid body"})
			return
	...
	filtered := make([]config.OrcaRouterKey, 0, len(arr))
	for i := range arr {
		entry := arr[i]
		normalizeCodexKey(&entry)
		if entry.BaseURL == "" {
			entry.BaseURL = config.OrcaRouterProviderBaseURL
		}
		if rejectInvalidCredentialWeight(c, fmt.Sprintf("orcarouter-api-key[%d].weight", i), entry.Weight) {
			return
		}
		filtered = append(filtered, entry)
	}
	h.mu.Lock()
	defer h.mu.Unlock()
	h.cfg.OrcaRouterKey = filtered
	h.cfg.SanitizeOrcaRouterKeys()
	h.persistLocked(c)
}
func (h *Handler) PatchOrcaRouterKey(c *gin.Context) {
	type orcaRouterKeyPatch struct {
		APIKey         *string                   `json:"api-key"`
		Priority       *int                      `json:"priority"`
		Weight         json.RawMessage           `json:"weight"`
		Prefix         *string                   `json:"prefix"`
		...
		Websockets     *bool                     `json:"websockets"`
		ProxyURL       *string                   `json:"proxy-url"`
		Models         *[]config.OrcaRouterModel `json:"models"`
		Headers        *map[string]string        `json:"headers"`
		ExcludedModels *[]string                 `json:"excluded-models"`
		DisableCooling *bool                     `json:"disable-cooling"`
	}
	var body struct {
		Index *int                `json:"index"`
		Match *string             `json:"match"`
		Value *orcaRouterKeyPatch `json:"value"`
	}
	if errBind := c.ShouldBindJSON(&body); errBind != nil || body.Value == nil {
		c.JSON(400, gin.H{"error": "invalid body"})
		return
	...
	defer h.mu.Unlock()
	targetIndex := -1
	if body.Index != nil && *body.Index >= 0 && *body.Index < len(h.cfg.OrcaRouterKey) {
		targetIndex = *body.Index
	}
	if targetIndex == -1 && body.Match != nil {
		match := strings.TrimSpace(*body.Match)
		for i := range h.cfg.OrcaRouterKey {
			if h.cfg.OrcaRouterKey[i].APIKey == match {
				targetIndex = i
				break
			}
		}
	...
	entry := h.cfg.OrcaRouterKey[targetIndex]
	if body.Value.APIKey != nil {
		entry.APIKey = strings.TrimSpace(*body.Value.APIKey)
	}
	if body.Value.Priority != nil {
	...
	if body.Value.Models != nil {
		entry.Models = append([]config.OrcaRouterModel(nil), (*body.Value.Models)...)
	}
	if body.Value.Headers != nil {
		entry.Headers = config.NormalizeHeaders(*body.Value.Headers)
	}
	...
	normalizeCodexKey(&entry)
	h.cfg.OrcaRouterKey[targetIndex] = entry
	h.cfg.SanitizeOrcaRouterKeys()
	h.persistLocked(c)
}

func (h *Handler) DeleteOrcaRouterKey(c *gin.Context) {
	h.mu.Lock()
	defer h.mu.Unlock()
	if val := strings.TrimSpace(c.Query("api-key")); val != "" {
		if baseRaw, okBase := c.GetQuery("base-url"); okBase {
			base := strings.TrimSpace(baseRaw)
			out := make([]config.OrcaRouterKey, 0, len(h.cfg.OrcaRouterKey))
			for _, entry := range h.cfg.OrcaRouterKey {
				if strings.TrimSpace(entry.APIKey) == val && strings.TrimSpace(entry.BaseURL) == base {
					continue
				}
				out = append(out, entry)
			}
			h.cfg.OrcaRouterKey = out
			h.cfg.SanitizeOrcaRouterKeys()
			h.persistLocked(c)
			return
		}
		...
		matchCount := 0
		for i := range h.cfg.OrcaRouterKey {
			if strings.TrimSpace(h.cfg.OrcaRouterKey[i].APIKey) == val {
				matchCount++
				if matchIndex == -1 {
					matchIndex = i
				}
		...
		if matchIndex != -1 {
			h.cfg.OrcaRouterKey = append(h.cfg.OrcaRouterKey[:matchIndex], h.cfg.OrcaRouterKey[matchIndex+1:]...)
		}
		h.cfg.SanitizeOrcaRouterKeys()
		h.persistLocked(c)
		return
	...
	if idxStr := c.Query("index"); idxStr != "" {
		var idx int
		_, errScan := fmt.Sscanf(idxStr, "%d", &idx)
		if errScan == nil && idx >= 0 && idx < len(h.cfg.OrcaRouterKey) {
			h.cfg.OrcaRouterKey = append(h.cfg.OrcaRouterKey[:idx], h.cfg.OrcaRouterKey[idx+1:]...)
			h.cfg.SanitizeOrcaRouterKeys()
			h.persistLocked(c)
			return
		}
