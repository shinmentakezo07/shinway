...
func TestParseRegistryValidatesRegistry(t *testing.T) {
	...
	registry, errParse := ParseRegistry([]byte(`{
		...
		"plugins": [{
			...
			"homepage": "https://github.com/author-name/shinway-sample-provider-plugin",
			"license": "MIT",
			"tags": ["provider"]
		}]
	}`))
...
func TestParseRegistryNormalizesPluginFields(t *testing.T) {
	...
	if plugin.ID != "sample-provider" || plugin.Version != "0.1.0" || plugin.Repository != "https://github.com/author-name/shinway-sample-provider-plugin" {
