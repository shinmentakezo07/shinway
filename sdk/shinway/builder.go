// Package shinway provides the core service implementation for the CLI Proxy API.
...
import (
	...
	configaccess "github.com/shinmentakezo07/shinway/v7/internal/access/config_access"
...
func (b *Builder) Build() (*Service, error) {
	if b.cfg == nil {
		return nil, fmt.Errorf("shinway: configuration is required")
	}
	if b.configPath == "" {
		return nil, fmt.Errorf("shinway: configuration path is required")
	}
	if errValidate := b.cfg.ValidateCredentialWeights(); errValidate != nil {
		return nil, fmt.Errorf("shinway: validate credential weights: %w", errValidate)
	}
	...
	if errResolvePluginsDir := b.cfg.ResolvePluginsDir(); errResolvePluginsDir != nil && b.cfg.Plugins.Enabled {
		return nil, fmt.Errorf("shinway: %w", errResolvePluginsDir)
	}
