...
import (
	...
	"github.com/shinmentakezo07/shinway/v7/internal/api"
...
func (s *Service) Run(ctx context.Context) error {
	if s == nil {
		return fmt.Errorf("shinway: service is nil")
	}
	if ctx == nil {
	...
	if !homeEnabled {
		...
		if errCreate != nil {
			return fmt.Errorf("shinway: failed to create watcher: %w", errCreate)
		}
		...
		if errStart := watcherWrapper.Start(watcherCtx); errStart != nil {
			return fmt.Errorf("shinway: failed to start watcher: %w", errStart)
		}
	...
	if err != nil {
		if os.IsNotExist(err) {
			if mkErr := os.MkdirAll(s.cfg.AuthDir, 0o755); mkErr != nil {
				return fmt.Errorf("shinway: failed to create auth directory %s: %w", s.cfg.AuthDir, mkErr)
			}
			log.Infof("created missing auth directory: %s", s.cfg.AuthDir)
			return nil
		}
		return fmt.Errorf("shinway: error checking auth directory %s: %w", s.cfg.AuthDir, err)
	}
	if !info.IsDir() {
		return fmt.Errorf("shinway: auth path exists but is not a directory: %s", s.cfg.AuthDir)
	}
	return nil
}
