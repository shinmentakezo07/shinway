...
func discardStreamChunks(ch <-chan shinwayexecutor.StreamChunk) {
	if ch == nil {
		return
	}
	go func() {
...
func streamErrorResult(headers http.Header, err error) *shinwayexecutor.StreamResult {
	ch := make(chan shinwayexecutor.StreamChunk, 1)
	ch <- shinwayexecutor.StreamChunk{Err: err}
	close(ch)
	return &shinwayexecutor.StreamResult{
		Headers: cloneHTTPHeader(headers),
		Chunks:  ch,
	}
}

func readStreamBootstrap(ctx context.Context, ch <-chan shinwayexecutor.StreamChunk) ([]shinwayexecutor.StreamChunk, bool, error) {
	if ch == nil {
		return nil, true, nil
	}
	buffered := make([]shinwayexecutor.StreamChunk, 0, 1)
	for {
		var (
			chunk shinwayexecutor.StreamChunk
			ok    bool
		)
		if ctx != nil {
			select {
...
func (m *Manager) wrapStreamResult(ctx context.Context, auth *Auth, provider, resultModel string, headers http.Header, buffered []shinwayexecutor.StreamChunk, remaining <-chan shinwayexecutor.StreamChunk, aliasResult OAuthModelAliasResult, ephemeralResult bool) *shinwayexecutor.StreamResult {
	...
	go func() {
		...
		emit := func(chunk shinwayexecutor.StreamChunk) bool {
		...
		if tail := finishForceMappedStreamChunks(rewriter); len(tail) > 0 {
			tailChunk := shinwayexecutor.StreamChunk{Payload: tail}
			if !emit(tailChunk) {
				return
			}
		}
	...
	return &shinwayexecutor.StreamResult{Headers: headers, Chunks: out}
}

func (m *Manager) executeStreamWithModelPool(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req shinwayexecutor.Request, opts shinwayexecutor.Options, routeModel, executionModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult, allowRetry bool, ephemeralResult bool) (*shinwayexecutor.StreamResult, error) {
	if executor == nil {
	...
	for idx, execModel := range execModels {
			...
			if allowRetry {
				if refreshed, okRefresh := m.tryRefreshAfterUnauthorized(ctx, auth, bootstrapErr, didRefreshOnUnauthorized); okRefresh {
					...
					if retryErr != nil {
						...
						streamResult = &shinwayexecutor.StreamResult{}
					} else {
		...
		if closed {
			closedCh := make(chan shinwayexecutor.StreamChunk)
			close(closedCh)
			remaining = closedCh
		}
