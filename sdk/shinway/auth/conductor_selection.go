...
func authSelectionEligibilityForRequest(ctx context.Context, opts shinwayexecutor.Options) authSelectionEligibility {
...
func schedulerOptions(opts shinwayexecutor.Options) pluginapi.SchedulerOptions {
...
func (m *Manager) pickViaBuiltinScheduler(ctx context.Context, strategy schedulerStrategy, provider string, providers []string, model string, opts shinwayexecutor.Options, tried map[string]struct{}) (*Auth, bool, error) {
	if m == nil || m.scheduler == nil {
...
func (m *Manager) pickViaPluginScheduler(ctx context.Context, scheduler PluginScheduler, provider string, providers []string, model string, opts shinwayexecutor.Options, tried map[string]struct{}, candidates []*Auth) (*Auth, bool, error) {
...
func (m *Manager) pickNextLegacy(ctx context.Context, provider, model string, opts shinwayexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	if m.HomeEnabled() {
...
func (m *Manager) SelectAuth(ctx context.Context, provider, model string, opts shinwayexecutor.Options) (*Auth, error) {
...
func (m *Manager) SelectAuthByKind(ctx context.Context, provider, model, requiredKind string, opts shinwayexecutor.Options) (*Auth, error) {
	if m != nil && m.HomeEnabled() {
		return nil, &Error{Code: "home_unavailable", Message: "legacy auth selection is unavailable while Home is enabled", HTTPStatus: http.StatusServiceUnavailable}
	}
...
func (m *Manager) SelectAuthWithCredentialPolicy(ctx context.Context, provider, model, policy string, opts shinwayexecutor.Options) (*Auth, error) {
	if m != nil && m.HomeEnabled() {
		return nil, &Error{Code: "home_unavailable", Message: "legacy auth selection is unavailable while Home is enabled", HTTPStatus: http.StatusServiceUnavailable}
	}
	...
	selected, _, errPick := m.pickNextLegacy(withCredentialPolicy(ctx, policy), provider, model, opts, nil)
	if errPick != nil {
		return nil, errPick
	}
...
func (m *Manager) SelectHomeAuthByKind(ctx context.Context, provider string, model string, requiredKind string, opts shinwayexecutor.Options) (*HomeDispatchSelection, error) {
	requiredKind = normalizeAuthKind(requiredKind)
	if requiredKind == "" {
		return nil, &Error{Code: "invalid_auth_kind", Message: "required auth kind is invalid", HTTPStatus: http.StatusBadRequest}
	}
...
func (m *Manager) SelectHomeAuthWithCredentialPolicy(ctx context.Context, provider, model, policy string, opts shinwayexecutor.Options) (*HomeDispatchSelection, error) {
	policy = normalizeCredentialPolicy(policy)
	if policy == "" {
		return nil, &Error{Code: "invalid_credential_policy", Message: "credential policy is invalid", HTTPStatus: http.StatusBadRequest}
	}
	...
	for {
		selectionOpts := withHomeAuthCount(opts, homeAuthCount)
		selection, errSelection := m.pickHomeDispatchSelection(withCredentialPolicy(ctx, policy), model, selectionOpts)
		if errSelection != nil {
			return nil, errSelection
		}
		providerMatches := strings.TrimSpace(provider) == "" || strings.EqualFold(strings.TrimSpace(selection.Provider), strings.TrimSpace(provider))
		if providerMatches && credentialPolicyAllows(policy, selection.Auth) {
			return selection, nil
		}
		authID := ""
		if selection.Auth != nil {
...
func (m *Manager) pickNext(ctx context.Context, provider, model string, opts shinwayexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	if m.HomeEnabled() {
		auth, exec, _, err := m.pickNextViaHome(ctx, model, opts, tried)
		return auth, exec, err
	}
...
func (m *Manager) pickNextMixedLegacy(ctx context.Context, providers []string, model string, opts shinwayexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	if m.HomeEnabled() {
...
func (m *Manager) pickNextMixed(ctx context.Context, providers []string, model string, opts shinwayexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	if m.HomeEnabled() {
		return m.pickNextViaHome(ctx, model, opts, tried)
	}
