...
func (schedulerTestExecutor) Execute(ctx context.Context, auth *Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	return shinwayexecutor.Response{}, nil
}

func (schedulerTestExecutor) ExecuteStream(ctx context.Context, auth *Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (*shinwayexecutor.StreamResult, error) {
	return nil, nil
}
...
func (schedulerTestExecutor) CountTokens(ctx context.Context, auth *Auth, req shinwayexecutor.Request, opts shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	return shinwayexecutor.Response{}, nil
}
...
func (s *trackingSelector) Pick(ctx context.Context, provider, model string, opts shinwayexecutor.Options, auths []*Auth) (*Auth, error) {
	s.calls++
...
func TestSchedulerPick_RoundRobinHighestPriority(t *testing.T) {
	...
	for index, wantID := range want {
		got, errPick := scheduler.pickSingle(context.Background(), "gemini", "", shinwayexecutor.Options{}, nil)
		if errPick != nil {
...
func TestManagerLoad_WeightedRoundRobinUsesPersistedMetadataWeight(t *testing.T) {
	...
	for index := 0; index < 60; index++ {
		got, errPick := manager.scheduler.pickSingle(context.Background(), "gemini", "", shinwayexecutor.Options{}, nil)
		if errPick != nil {
			t.Fatalf("pickSingle() #%d error = %v", index, errPick)
		}
		counts[got.ID]++
...
func TestSchedulerPick_WeightedRoundRobinResetsCreditsWhenWeightsChange(t *testing.T) {
	...
	for index := 0; index < 1000; index++ {
		if _, errPick := scheduler.pickSingle(context.Background(), "gemini", "", shinwayexecutor.Options{}, nil); errPick != nil {
...
func TestSchedulerPick_WeightedWebsocketResetsCreditsWhenWeightsChange(t *testing.T) {
	...
	for index := 0; index < 1000; index++ {
		if _, errPick := scheduler.pickSingle(ctx, "codex", "", shinwayexecutor.Options{}, nil); errPick != nil {
	...
	counts := make(map[string]int)
	for index := 0; index < 20; index++ {
		got, errPick := scheduler.pickSingle(ctx, "codex", "", shinwayexecutor.Options{}, nil)
		if errPick != nil {
...
func TestManagerLegacyWeightedRoundRobinKeepsIndependentAliasPrefixedModelState(t *testing.T) {
	...
	for index := 0; index < 40; index++ {
		for _, model := range []string{"team-a/shared", "team-b/shared"} {
			got, _, errPick := manager.pickNext(context.Background(), "gemini", model, shinwayexecutor.Options{}, nil)
			if errPick != nil {
...
func TestSchedulerPick_PromotesExpiredCooldownBeforePick(t *testing.T) {
	...
	got, errPick := scheduler.pickSingle(context.Background(), "gemini", model, shinwayexecutor.Options{}, nil)
	if errPick != nil {
		t.Fatalf("pickSingle() error = %v", errPick)
	}
	if got == nil {
...
func TestSchedulerPick_XAIWebsocketPrefersWebsocketEnabledSubset(t *testing.T) {
	...
	for index, wantID := range want {
		got, errPick := scheduler.pickSingle(ctx, "xai", "", shinwayexecutor.Options{}, nil)
		if errPick != nil {
			t.Fatalf("pickSingle() #%d error = %v", index, errPick)
		}
		if got == nil {
...
func TestSchedulerPick_MixedProvidersUsesWeightedProviderRotationOverReadyCandidates(t *testing.T) {
	...
	for index := range wantProviders {
		got, provider, errPick := scheduler.pickMixed(context.Background(), []string{"gemini", "claude"}, "", shinwayexecutor.Options{}, nil)
		if errPick != nil {
...
func TestSchedulerPick_MixedProvidersResetsCreditsWhenWeightsChange(t *testing.T) {
	...
	for index := 0; index < 1000; index++ {
		if _, _, errPick := scheduler.pickMixed(context.Background(), providers, "", shinwayexecutor.Options{}, nil); errPick != nil {
	...
	counts := make(map[string]int)
	for index := 0; index < 20; index++ {
		got, _, errPick := scheduler.pickMixed(context.Background(), providers, "", shinwayexecutor.Options{}, nil)
		if errPick != nil {
...
func TestSchedulerPick_MixedProvidersPrefersHighestPriorityTier(t *testing.T) {
	...
	for index := range wantProviders {
		got, provider, errPick := scheduler.pickMixed(context.Background(), providers, model, shinwayexecutor.Options{}, nil)
		if errPick != nil {
			t.Fatalf("pickMixed() #%d error = %v", index, errPick)
		}
		if got == nil {
...
func TestManager_PickNextMixed_UsesWeightedProviderRotationBeforeCredentialRotation(t *testing.T) {
	...
	wantIDs := []string{"gemini-a", "gemini-b", "claude-a", "gemini-a"}
	for index := range wantProviders {
		got, _, provider, errPick := manager.pickNextMixed(context.Background(), []string{"gemini", "claude"}, "", shinwayexecutor.Options{}, map[string]struct{}{})
		if errPick != nil {
...
func TestManager_PickNextMixed_DisallowFreeAuthSkipsCodexFreePlan(t *testing.T) {
	...
	opts := shinwayexecutor.Options{
		Metadata: map[string]any{shinwayexecutor.DisallowFreeAuthMetadataKey: true},
	}
...
func TestManagerPluginSchedulerSelectsAuthID(t *testing.T) {
	...
	got, _, errPick := manager.pickNext(context.Background(), "gemini", "", shinwayexecutor.Options{Stream: true}, nil)
	if errPick != nil {
...
func TestManagerSelectAuthByKindSkipsAPIKey(t *testing.T) {
	...
	selected, errSelect := manager.SelectAuthByKind(context.Background(), "codex", "", AuthKindOAuth, shinwayexecutor.Options{})
	if errSelect != nil {
...
func TestManagerWeightedRoundRobinDisallowFreeAuthIgnoresFreeWeight(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			opts := shinwayexecutor.Options{Metadata: map[string]any{shinwayexecutor.DisallowFreeAuthMetadataKey: true}}
			counts := make(map[string]int)
			for index := 0; index < 600; index++ {
				var selected *Auth
				var errPick error
...
func TestManagerSelectAuthByKindRejectsInvalidKind(t *testing.T) {
	manager := NewManager(nil, &RoundRobinSelector{}, nil)
	selected, errSelect := manager.SelectAuthByKind(context.Background(), "codex", "", "certificate", shinwayexecutor.Options{})
	if selected != nil {
		t.Fatalf("SelectAuthByKind() auth = %#v, want nil", selected)
	}
	var authErr *Error
...
func TestManagerLegacySelectAuthFailsClosedWhenHomeEnabled(t *testing.T) {
	...
	for name, selectAuth := range map[string]func() (*Auth, error){
		"SelectAuth": func() (*Auth, error) {
			return manager.SelectAuth(context.Background(), "test", "model", shinwayexecutor.Options{})
		},
		"SelectAuthByKind": func() (*Auth, error) {
			return manager.SelectAuthByKind(context.Background(), "test", "model", AuthKindOAuth, shinwayexecutor.Options{})
		},
	} {
...
func TestSelectHomeAuthByKindReturnsHomeSelection(t *testing.T) {
	...
	selection, errSelect := manager.SelectHomeAuthByKind(context.Background(), "test", "gpt-5.4", AuthKindOAuth, shinwayexecutor.Options{})
	if errSelect != nil {
...
func TestSelectHomeAuthByKindKeepsLogicalProviderWhenUsingCompatibilityExecutor(t *testing.T) {
	...
	selection, errSelect := manager.SelectHomeAuthByKind(context.Background(), "base-url-provider", "gpt-5.4", AuthKindAPIKey, shinwayexecutor.Options{})
	if errSelect != nil {
		t.Fatalf("SelectHomeAuthByKind() error = %v", errSelect)
	}
...
func TestPickNextViaHomeEndsPendingOnInvalidAuth(t *testing.T) {
	...
	_, _, _, errPick := manager.pickNextViaHome(context.Background(), "gpt-5.4", shinwayexecutor.Options{}, nil)
	var authErr *Error
...
func TestManagerPluginSchedulerSkippedWhenHomeEnabled(t *testing.T) {
	...
	_, _, _ = manager.pickNext(context.Background(), "gemini", "", shinwayexecutor.Options{}, nil)
...
func TestManagerInactivePluginSchedulerKeepsFastPath(t *testing.T) {
	...
	gotA, _, errPick := manager.pickNext(context.Background(), "gemini", "", shinwayexecutor.Options{}, nil)
	if errPick != nil {
		t.Fatalf("pickNext() first error = %v", errPick)
	}
	gotB, _, errPick := manager.pickNext(context.Background(), "gemini", "", shinwayexecutor.Options{}, nil)
	if errPick != nil {
...
func TestManagerPluginSchedulerCalledOutsideManagerLock(t *testing.T) {
	...
	got, _, errPick := manager.pickNext(context.Background(), "gemini", "", shinwayexecutor.Options{}, nil)
	if errPick != nil {
		t.Fatalf("pickNext() error = %v", errPick)
	}
	if got == nil {
...
func TestManagerPluginSchedulerDelegatesBuiltin(t *testing.T) {
	...
	t.Run("round-robin model cursors", func(t *testing.T) {
		...
		gotModelA, _, errPick := manager.pickNext(context.Background(), "gemini", "model-a", shinwayexecutor.Options{}, nil)
		if errPick != nil {
			t.Fatalf("pickNext(model-a) error = %v", errPick)
		}
		gotModelB, _, errPick := manager.pickNext(context.Background(), "gemini", "model-b", shinwayexecutor.Options{}, nil)
		if errPick != nil {
...
func TestManagerPluginSchedulerDelegateRoundRobinUsesNativeMixedRotation(t *testing.T) {
	...
	wantIDs := []string{"gemini-a", "gemini-b", "claude-a", "gemini-a"}
	for index := range wantProviders {
		got, _, provider, errPick := manager.pickNextMixed(context.Background(), []string{"gemini", "claude"}, "", shinwayexecutor.Options{}, nil)
		if errPick != nil {
			t.Fatalf("pickNextMixed() #%d error = %v", index, errPick)
		}
		if got == nil {
...
func TestManagerPluginSchedulerPickNextMixedSelectsProvider(t *testing.T) {
	...
	got, executor, provider, errPick := manager.pickNextMixed(context.Background(), []string{"gemini", "claude"}, "", shinwayexecutor.Options{}, nil)
	if errPick != nil {
		t.Fatalf("pickNextMixed() error = %v", errPick)
	}
	if got == nil {
...
func TestManagerPluginSchedulerCandidatesAreSafeCopies(t *testing.T) {
	...
	if _, _, errPick := manager.pickNext(context.Background(), "gemini", "", shinwayexecutor.Options{}, nil); errPick != nil {
		t.Fatalf("pickNext() error = %v", errPick)
	}
...
func TestManagerCustomSelector_FallsBackToLegacyPath(t *testing.T) {
	...
	got, _, errPick := manager.pickNext(context.Background(), "gemini", "", shinwayexecutor.Options{}, map[string]struct{}{})
	if errPick != nil {
		t.Fatalf("pickNext() error = %v", errPick)
	}
	if got == nil {
...
func TestManager_SchedulerTracksMarkResultCooldownAndRecovery(t *testing.T) {
	...
	got, errPick := manager.scheduler.pickSingle(context.Background(), "gemini", "test-model", shinwayexecutor.Options{}, nil)
	if errPick != nil {
