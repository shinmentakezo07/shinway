...
func TestHomeBusySkipsNormalAndStreamOuterRetries(t *testing.T) {
	for _, stream := range []bool{false, true} {
		t.Run(map[bool]string{false: "normal", true: "stream"}[stream], func(t *testing.T) {
			...
			go func() {
				if stream {
					_, errExecute := manager.ExecuteStream(ctx, []string{"home-busy"}, shinwayexecutor.Request{Model: "gpt"}, shinwayexecutor.Options{Stream: true})
					result <- errExecute
					return
				}
				_, errExecute := manager.Execute(ctx, []string{"home-busy"}, shinwayexecutor.Request{Model: "gpt"}, shinwayexecutor.Options{})
				result <- errExecute
			}()
...
func TestPickHomeDispatchSelectionReleasesAccountedScopeAfterAuthValidationFailure(t *testing.T) {
	...
	selection, errPick := manager.pickHomeDispatchSelection(context.Background(), "gpt", shinwayexecutor.Options{})
