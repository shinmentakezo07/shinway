...
func (e *forceMappingExecutor) Execute(_ context.Context, _ *Auth, req shinwayexecutor.Request, _ shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	e.mu.Lock()
	e.executeModels = append(e.executeModels, req.Model)
	e.mu.Unlock()
	payload := forceMappingNonStreamUpstreamPayload(e.id, req.Model)
	return shinwayexecutor.Response{Payload: []byte(payload)}, nil
}

func (e *forceMappingExecutor) ExecuteStream(_ context.Context, _ *Auth, req shinwayexecutor.Request, _ shinwayexecutor.Options) (*shinwayexecutor.StreamResult, error) {
	e.mu.Lock()
	e.streamModels = append(e.streamModels, req.Model)
	e.mu.Unlock()
	chunks := forceMappingStreamUpstreamChunks(e.id, req.Model)
	ch := make(chan shinwayexecutor.StreamChunk, len(chunks))
	for _, chunk := range chunks {
		ch <- shinwayexecutor.StreamChunk{Payload: chunk}
	}
	close(ch)
	return &shinwayexecutor.StreamResult{Chunks: ch}, nil
}
...
func (e *forceMappingExecutor) CountTokens(context.Context, *Auth, shinwayexecutor.Request, shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	return shinwayexecutor.Response{}, &Error{HTTPStatus: http.StatusNotImplemented, Message: "CountTokens not implemented"}
}
...
func (e *forceMappingCreditsFallbackExecutor) Execute(ctx context.Context, _ *Auth, req shinwayexecutor.Request, _ shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	...
	e.mu.Unlock()
	if !creditsRequested {
		return shinwayexecutor.Response{}, &Error{HTTPStatus: http.StatusServiceUnavailable, Message: "MODEL_CAPACITY_EXHAUSTED"}
	}
	payload := `{"model":"` + req.Model + `","message":{"model":"` + req.Model + `"}}`
	return shinwayexecutor.Response{Payload: []byte(payload)}, nil
}

func (e *forceMappingCreditsFallbackExecutor) ExecuteStream(ctx context.Context, _ *Auth, req shinwayexecutor.Request, _ shinwayexecutor.Options) (*shinwayexecutor.StreamResult, error) {
	creditsRequested := AntigravityCreditsRequested(ctx)
	e.mu.Lock()
	e.streamModels = append(e.streamModels, req.Model)
	...
	if !creditsRequested {
		ch <- shinwayexecutor.StreamChunk{Err: &Error{HTTPStatus: http.StatusServiceUnavailable, Message: "MODEL_CAPACITY_EXHAUSTED"}}
		close(ch)
		return &shinwayexecutor.StreamResult{Chunks: ch}, nil
	}
	ch <- shinwayexecutor.StreamChunk{Payload: []byte(`data: {"message":{"model":"` + req.Model + `"}}` + "\n\n")}
	close(ch)
	return &shinwayexecutor.StreamResult{Chunks: ch}, nil
}
...
func (e *forceMappingCreditsFallbackExecutor) CountTokens(context.Context, *Auth, shinwayexecutor.Request, shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	return shinwayexecutor.Response{}, &Error{HTTPStatus: http.StatusNotImplemented, Message: "CountTokens not implemented"}
}
...
func TestManagerExecute_OAuthAliasForceMappingRewritesNonStreamResponse(t *testing.T) {
	...
	resp, errExecute := manager.Execute(context.Background(), []string{provider}, shinwayexecutor.Request{Model: aliasModel}, shinwayexecutor.Options{})
	if errExecute != nil {
...
func TestManagerExecuteStream_OAuthAliasForceMappingRewritesStreamResponse(t *testing.T) {
	...
	manager, executor := setupForceMappingManager(t, provider, upstreamModel, aliasModel)
	streamResult, errExecute := manager.ExecuteStream(context.Background(), []string{provider}, shinwayexecutor.Request{Model: aliasModel}, shinwayexecutor.Options{})
	if errExecute != nil {
...
func TestManagerExecute_OAuthAliasForceMappingRewritesKimiAndXAIResponses(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.provider, func(t *testing.T) {
			...
			resp, errExecute := manager.Execute(context.Background(), []string{tc.provider}, shinwayexecutor.Request{Model: tc.aliasModel}, shinwayexecutor.Options{})
			if errExecute != nil {
				t.Fatalf("execute error = %v, want success", errExecute)
			}
			gotModels := executor.ExecuteModels()
...
func TestManagerExecuteStream_OAuthAliasForceMappingRewritesKimiAndXAIResponses(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.provider, func(t *testing.T) {
			manager, executor := setupForceMappingManager(t, tc.provider, tc.upstreamModel, tc.aliasModel)
			streamResult, errExecute := manager.ExecuteStream(context.Background(), []string{tc.provider}, shinwayexecutor.Request{Model: tc.aliasModel}, shinwayexecutor.Options{})
			if errExecute != nil {
				t.Fatalf("execute stream error = %v, want success", errExecute)
			}
			gotModels := executor.StreamModels()
...
func TestManagerExecute_AntigravityCreditsFallbackForceMappingRewritesResponse(t *testing.T) {
	...
	resp, errExecute := manager.Execute(context.Background(), []string{"antigravity"}, shinwayexecutor.Request{Model: aliasModel}, shinwayexecutor.Options{})
	if errExecute != nil {
		t.Fatalf("execute error = %v, want success", errExecute)
	}
...
func TestManagerExecuteStream_AntigravityCreditsFallbackForceMappingRewritesResponse(t *testing.T) {
	...
	manager, executor := setupForceMappingCreditsFallbackManager(t, upstreamModel, aliasModel)
	streamResult, errExecute := manager.ExecuteStream(context.Background(), []string{"antigravity"}, shinwayexecutor.Request{Model: aliasModel}, shinwayexecutor.Options{})
	if errExecute != nil {
		t.Fatalf("execute stream error = %v, want success", errExecute)
	}
...
func TestManagerExecute_APIKeyAliasForceMappingRewritesResponse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.provider, func(t *testing.T) {
			...
			resp, errExecute := manager.Execute(context.Background(), []string{tt.provider}, shinwayexecutor.Request{Model: tt.aliasModel}, shinwayexecutor.Options{})
			if errExecute != nil {
				t.Fatalf("execute error = %v, want success", errExecute)
			}
...
func TestManagerExecuteStream_APIKeyAliasForceMappingRewritesResponse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.provider, func(t *testing.T) {
			manager, executor := setupAPIKeyForceMappingManager(t, tt.provider, tt.upstreamModel, tt.aliasModel)
			streamResult, errExecute := manager.ExecuteStream(context.Background(), []string{tt.provider}, shinwayexecutor.Request{Model: tt.aliasModel}, shinwayexecutor.Options{})
			if errExecute != nil {
				t.Fatalf("execute stream error = %v, want success", errExecute)
			}
