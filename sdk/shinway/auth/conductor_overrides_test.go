...
func (e *credentialRetryLimitExecutor) Execute(context.Context, *Auth, shinwayexecutor.Request, shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	e.recordCall()
	return shinwayexecutor.Response{}, &Error{HTTPStatus: 500, Message: "boom"}
}

func (e *credentialRetryLimitExecutor) ExecuteStream(context.Context, *Auth, shinwayexecutor.Request, shinwayexecutor.Options) (*shinwayexecutor.StreamResult, error) {
	e.recordCall()
...
func (e *credentialRetryLimitExecutor) CountTokens(context.Context, *Auth, shinwayexecutor.Request, shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	e.recordCall()
	return shinwayexecutor.Response{}, &Error{HTTPStatus: 500, Message: "boom"}
}
...
func (e *authFallbackExecutor) Execute(_ context.Context, auth *Auth, _ shinwayexecutor.Request, _ shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	e.mu.Lock()
	...
	return shinwayexecutor.Response{Payload: []byte(auth.ID)}, nil
}

func (e *authFallbackExecutor) ExecuteStream(_ context.Context, auth *Auth, _ shinwayexecutor.Request, _ shinwayexecutor.Options) (*shinwayexecutor.StreamResult, error) {
	e.mu.Lock()
	e.streamCalls = append(e.streamCalls, auth.ID)
	err := e.streamFirstErrors[auth.ID]
	e.mu.Unlock()

	ch := make(chan shinwayexecutor.StreamChunk, 1)
	if err != nil {
		ch <- shinwayexecutor.StreamChunk{Err: err}
		close(ch)
		return &shinwayexecutor.StreamResult{Headers: http.Header{"X-Auth": {auth.ID}}, Chunks: ch}, nil
	}
	ch <- shinwayexecutor.StreamChunk{Payload: []byte(auth.ID)}
	close(ch)
	return &shinwayexecutor.StreamResult{Headers: http.Header{"X-Auth": {auth.ID}}, Chunks: ch}, nil
}
...
func (e *authFallbackExecutor) CountTokens(_ context.Context, auth *Auth, _ shinwayexecutor.Request, _ shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	e.mu.Lock()
...
func TestManager_MaxRetryCredentials_LimitsCrossCredentialRetries(t *testing.T) {
	request := shinwayexecutor.Request{Model: "test-model"}
	testCases := []struct {
		name   string
	...
	}{
		{
			...
			invoke: func(m *Manager) error {
				_, errExecute := m.Execute(context.Background(), []string{"claude"}, request, shinwayexecutor.Options{})
				return errExecute
			},
		},
		{
			name: "execute_count",
			invoke: func(m *Manager) error {
				_, errExecute := m.ExecuteCount(context.Background(), []string{"claude"}, request, shinwayexecutor.Options{})
				return errExecute
			},
		},
		{
			name: "execute_stream",
			invoke: func(m *Manager) error {
				_, errExecute := m.ExecuteStream(context.Background(), []string{"claude"}, request, shinwayexecutor.Options{})
				return errExecute
			},
		},
	}
...
func TestManager_ModelSupportBadRequest_FallsBackAndSuspendsAuth(t *testing.T) {
	...
	request := shinwayexecutor.Request{Model: model}
	for i := 0; i < 2; i++ {
		resp, errExecute := m.Execute(context.Background(), []string{"claude"}, request, shinwayexecutor.Options{})
		if errExecute != nil {
...
func TestManagerExecute_AntigravityInvalidGrantFallsBackAndSuspendsAuth(t *testing.T) {
	...
	request := shinwayexecutor.Request{Model: model}
	for i := 0; i < 2; i++ {
		resp, errExecute := m.Execute(context.Background(), []string{"antigravity"}, request, shinwayexecutor.Options{})
		if errExecute != nil {
			t.Fatalf("execute %d error = %v, want success", i, errExecute)
		}
		if string(resp.Payload) != goodAuth.ID {
...
func TestManagerExecuteStream_AntigravityInvalidGrantFallsBackAndSuspendsAuth(t *testing.T) {
	...
	request := shinwayexecutor.Request{Model: model}
	for i := 0; i < 2; i++ {
		streamResult, errExecute := m.ExecuteStream(context.Background(), []string{"antigravity"}, request, shinwayexecutor.Options{})
		if errExecute != nil {
...
func TestManagerExecuteStream_ModelSupportBadRequestFallsBackAndSuspendsAuth(t *testing.T) {
	...
	request := shinwayexecutor.Request{Model: model}
	for i := 0; i < 2; i++ {
		streamResult, errExecute := m.ExecuteStream(context.Background(), []string{"claude"}, request, shinwayexecutor.Options{})
		if errExecute != nil {
			t.Fatalf("execute stream %d error = %v, want success", i, errExecute)
		}
		var payload []byte
...
func TestManager_Execute_DisableCooling_DoesNotBlackoutAfter403(t *testing.T) {
	...
	req := shinwayexecutor.Request{Model: model}
	_, errExecute1 := m.Execute(context.Background(), []string{"claude"}, req, shinwayexecutor.Options{})
	if errExecute1 == nil {
	...
	_, errExecute2 := m.Execute(context.Background(), []string{"claude"}, req, shinwayexecutor.Options{})
	if errExecute2 == nil {
...
func TestManager_Execute_DisableCooling_RetriesAfter429RetryAfter(t *testing.T) {
	...
	req := shinwayexecutor.Request{Model: model}
	_, errExecute := m.Execute(context.Background(), []string{"claude"}, req, shinwayexecutor.Options{})
	if errExecute == nil {
...
func TestManager_RequestScopedErrorStopsCredentialFallbackWithoutSuspendingAuth(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.stream {
				result, errStream := m.ExecuteStream(context.Background(), []string{provider}, shinwayexecutor.Request{Model: model}, shinwayexecutor.Options{Stream: true})
				if result != nil {
				...
				_, errExecute = m.Execute(context.Background(), []string{provider}, shinwayexecutor.Request{Model: model}, shinwayexecutor.Options{})
			}
			if errExecute == nil {
...
func TestManager_ExecuteCount_GenericRouteNotFoundDoesNotSuspendModel(t *testing.T) {
	...
	if _, errCount := m.ExecuteCount(context.Background(), []string{"claude"}, shinwayexecutor.Request{Model: model}, shinwayexecutor.Options{}); errCount == nil {
	...
	resp, errExecute := m.Execute(context.Background(), []string{"claude"}, shinwayexecutor.Request{Model: model}, shinwayexecutor.Options{})
	if errExecute != nil {
...
func TestManager_Execute_GenericRouteNotFoundStillSuspendsModel(t *testing.T) {
	...
	if _, errExecute := m.Execute(context.Background(), []string{"claude"}, shinwayexecutor.Request{Model: model}, shinwayexecutor.Options{}); errExecute == nil {
...
func TestManager_RequestScopedNotFoundStopsRetryWithoutSuspendingAuth(t *testing.T) {
	...
	_, errExecute := m.Execute(context.Background(), []string{"openai"}, shinwayexecutor.Request{Model: model}, shinwayexecutor.Options{})
	if errExecute == nil {
