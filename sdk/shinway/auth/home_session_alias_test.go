...
func TestHomeSessionAliasCacheClearsWhenConfiguredTTLChanges(t *testing.T) {
	...
	combined := shinwayexecutor.Options{OriginalRequest: []byte(
		`{"conversation":{"id":"ttl-conversation"},"prompt_cache_key":"ttl-prompt"}`,
	)}
	conversationOnly := shinwayexecutor.Options{OriginalRequest: []byte(
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, payload := range tt.payloads {
				selection, errSelection := manager.pickHomeDispatchSelection(context.Background(), "gpt-test", shinwayexecutor.Options{
