...
const (
	homeAuthCountMetadataKey = "__shinway_home_auth_count"
...
func homeExecutionSessionIDFromMetadata(meta map[string]any) string {
	...
	raw, ok := meta[shinwayexecutor.ExecutionSessionMetadataKey]
	if !ok || raw == nil {
		return ""
	}
...
func (m *Manager) lockHomeWebsocketSession(ctx context.Context, opts shinwayexecutor.Options) func() {
	if m == nil || !shinwayexecutor.DownstreamWebsocket(ctx) {
		return nil
	}
...
func (m *Manager) retainedHomeSessionSelection(ctx context.Context, opts shinwayexecutor.Options, model string) (*HomeDispatchSelection, bool, error) {
	if m == nil || !shinwayexecutor.DownstreamWebsocket(ctx) {
...
func (m *Manager) retainHomeWebsocketSelection(ctx context.Context, opts shinwayexecutor.Options, model string, selection *HomeDispatchSelection) bool {
	if m == nil || selection == nil || !selection.Retained() || !shinwayexecutor.DownstreamWebsocket(ctx) || selection.Auth == nil {
		return false
	}
	sessionID := homeExecutionSessionIDFromMetadata(opts.Metadata)
...
func (m *Manager) bindHomeSelectionRuntimeAuth(ctx context.Context, opts shinwayexecutor.Options, selection *HomeDispatchSelection) error {
	if m == nil || selection == nil || !shinwayexecutor.DownstreamWebsocket(ctx) || selection.Auth == nil || !authWebsocketsEnabled(selection.Auth) {
		return nil
	}
	sessionID := homeExecutionSessionIDFromMetadata(opts.Metadata)
...
func (m *Manager) pickNextViaHome(ctx context.Context, model string, opts shinwayexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	if m == nil {
...
func (m *Manager) pickHomeDispatchSelection(ctx context.Context, model string, opts shinwayexecutor.Options) (*HomeDispatchSelection, error) {
	if m == nil {
	...
	if executionSessionID := homeExecutionSessionIDFromMetadata(opts.Metadata); executionSessionID != "" && shinwayexecutor.DownstreamWebsocket(ctx) {
...
func requestedModelFromMetadata(metadata map[string]any, fallback string) string {
	if metadata != nil {
		if v, ok := metadata[shinwayexecutor.RequestedModelMetadataKey]; ok {
			switch typed := v.(type) {
			case string:
...
func (m *Manager) findAllAntigravityCreditsCandidateAuths(ctx context.Context, routeModel string, opts shinwayexecutor.Options) ([]creditsCandidateEntry, error) {
...
func (m *Manager) tryAntigravityCreditsExecute(ctx context.Context, req shinwayexecutor.Request, opts shinwayexecutor.Options) (shinwayexecutor.Response, bool, error) {
	if m != nil && m.HomeEnabled() {
		return shinwayexecutor.Response{}, false, &Error{Code: "home_fallback_unsupported", Message: "Home does not support Antigravity credits fallback", HTTPStatus: http.StatusServiceUnavailable}
	}
	if !m.localExecutionAllowed() {
		return shinwayexecutor.Response{}, false, nil
	}
	routeModel := req.Model
	candidates, errCandidates := m.findAllAntigravityCreditsCandidateAuths(ctx, routeModel, opts)
	if errCandidates != nil {
		return shinwayexecutor.Response{}, false, errCandidates
	}
	for _, c := range candidates {
		...
		if rt := m.roundTripperFor(c.auth); rt != nil {
			...
			creditsCtx = context.WithValue(creditsCtx, "shinway.roundtripper", rt)
		}
...
func (m *Manager) tryAntigravityCreditsExecuteStream(ctx context.Context, req shinwayexecutor.Request, opts shinwayexecutor.Options) (*shinwayexecutor.StreamResult, bool, error) {
	if m != nil && m.HomeEnabled() {
