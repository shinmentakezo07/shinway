...
func (e *requestPrepareExecutor) Execute(_ context.Context, auth *Auth, _ shinwayexecutor.Request, _ shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	if errPrepared := e.recordPreparedAuth(auth); errPrepared != nil {
		return shinwayexecutor.Response{}, errPrepared
	}
	if e.executeErr != nil {
		return shinwayexecutor.Response{}, e.executeErr
	}
	return shinwayexecutor.Response{Payload: []byte("ok")}, nil
}

func (e *requestPrepareExecutor) ExecuteStream(_ context.Context, auth *Auth, _ shinwayexecutor.Request, _ shinwayexecutor.Options) (*shinwayexecutor.StreamResult, error) {
	if errPrepared := e.recordPreparedAuth(auth); errPrepared != nil {
...
func (e *requestPrepareExecutor) CountTokens(_ context.Context, auth *Auth, _ shinwayexecutor.Request, _ shinwayexecutor.Options) (shinwayexecutor.Response, error) {
	if errPrepared := e.recordPreparedAuth(auth); errPrepared != nil {
		return shinwayexecutor.Response{}, errPrepared
	}
	if e.executeErr != nil {
	...
	for _, path := range []struct {
	...
	}{
		{
			...
			run: func(manager *Manager, ctx context.Context) error {
				_, errExecute := manager.Execute(ctx, []string{"antigravity"}, shinwayexecutor.Request{Model: "test-model"}, shinwayexecutor.Options{})
				return errExecute
			},
		},
		{
			name: "Count",
			run: func(manager *Manager, ctx context.Context) error {
				_, errCount := manager.ExecuteCount(ctx, []string{"antigravity"}, shinwayexecutor.Request{Model: "test-model"}, shinwayexecutor.Options{})
				return errCount
			},
		},
		{
			name: "Stream",
			run: func(manager *Manager, ctx context.Context) error {
				result, errStream := manager.ExecuteStream(ctx, []string{"antigravity"}, shinwayexecutor.Request{Model: "test-model"}, shinwayexecutor.Options{Stream: true})
				if errStream != nil {
					return errStream
				}
...
func TestManagerExecute_PreparesAndPersistsMissingRequestAuthMetadata(t *testing.T) {
	...
	resp, errExecute := manager.Execute(context.Background(), []string{"antigravity"}, shinwayexecutor.Request{Model: model}, shinwayexecutor.Options{})
	if errExecute != nil {
	...
	if _, errExecute = manager.Execute(context.Background(), []string{"antigravity"}, shinwayexecutor.Request{Model: model}, shinwayexecutor.Options{}); errExecute != nil {
