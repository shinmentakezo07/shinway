...
import (
	...
	"github.com/shinmentakezo07/shinway/v7/internal/credentialweight"
...
func (s *RoundRobinSelector) Pick(ctx context.Context, provider, model string, opts shinwayexecutor.Options, auths []*Auth) (*Auth, error) {
	_ = opts
	now := time.Now()
...
func (s *WeightedRoundRobinSelector) Pick(ctx context.Context, provider, model string, opts shinwayexecutor.Options, auths []*Auth) (*Auth, error) {
	_ = opts
...
func (s *FillFirstSelector) Pick(ctx context.Context, provider, model string, opts shinwayexecutor.Options, auths []*Auth) (*Auth, error) {
	_ = opts
	now := time.Now()
	available, err := getAvailableAuths(auths, provider, model, now)
	if err != nil {
...
func (s *SessionAffinitySelector) Pick(ctx context.Context, provider, model string, opts shinwayexecutor.Options, auths []*Auth) (*Auth, error) {
...
func normalizedSessionCandidate(raw string) string {
	return shinwaysession.NormalizeExplicitID(raw)
}
...
func extractSessionIDs(headers http.Header, payload []byte, metadata map[string]any) (string, string) {
	...
	if sid := shinwaysession.ClaudeMetadataSessionID(payload); sid != "" {
		return "claude:" + sid, ""
	}
	...
	if executionID, ok := metadata[shinwayexecutor.ExecutionSessionMetadataKey].(string); ok {
		if executionID = normalizedSessionCandidate(executionID); executionID != "" {
			return "execution:" + executionID, ""
		}
	}
	if derivedID := normalizedSessionCandidate(shinwaysession.DerivedID(metadata)); derivedID != "" {
