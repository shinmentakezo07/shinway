...
func TestEnrichSkipsDerivationForExplicitSessions(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			payload:        []byte(`{"messages":[{"role":"user","content":"hello"}]}`),
			optionMetadata: map[string]any{shinwayexecutor.ExecutionSessionMetadataKey: "execution-session"},
		},
		{
			name:            "execution session request metadata",
			payload:         []byte(`{"messages":[{"role":"user","content":"hello"}]}`),
			requestMetadata: map[string]any{shinwayexecutor.ExecutionSessionMetadataKey: "execution-session"},
		},
		{
			...
			headers: http.Header{"x-session-id": []string{"header-session"}},
			optionMetadata: map[string]any{
				shinwayexecutor.DerivedSessionIDMetadataKey: "ctx:v1:stale",
			},
		},
	}
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			req := shinwayexecutor.Request{Payload: test.payload, Metadata: test.requestMetadata}
			...
			if test.name == "execution session option metadata" || test.name == "execution session request metadata" {
				if got := metadataString(enrichedReq.Metadata, shinwayexecutor.ExecutionSessionMetadataKey); got != "execution-session" {
					t.Fatalf("request execution session = %q, want execution-session", got)
				}
				if got := metadataString(enrichedOpts.Metadata, shinwayexecutor.ExecutionSessionMetadataKey); got != "execution-session" {
...
func TestEnrichDerivesAfterInvalidSessionIdentity(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			optionMetadata: map[string]any{shinwayexecutor.ExecutionSessionMetadataKey: strings.Repeat("x", 257)},
		},
		{
			name:            "control character execution session request metadata",
			payload:         []byte(`{"input":"hello"}`),
			requestMetadata: map[string]any{shinwayexecutor.ExecutionSessionMetadataKey: "bad\nsession"},
		},
		{
			name:           "oversized retained derived session option metadata",
			payload:        []byte(`{"input":"hello"}`),
			optionMetadata: map[string]any{shinwayexecutor.DerivedSessionIDMetadataKey: strings.Repeat("x", 257)},
		},
		{
			name:            "control character retained derived session request metadata",
			payload:         []byte(`{"input":"hello"}`),
			requestMetadata: map[string]any{shinwayexecutor.DerivedSessionIDMetadataKey: "bad\nsession"},
		},
	}
	for _, test := range tests {
		test := test
		t.Run(test.name, func(t *testing.T) {
			...
			if got := metadataString(enrichedReq.Metadata, shinwayexecutor.ExecutionSessionMetadataKey); got != "" {
				t.Fatalf("request execution session = %q, want invalid value removed", got)
			}
			if got := metadataString(enrichedOpts.Metadata, shinwayexecutor.ExecutionSessionMetadataKey); got != "" {
...
func TestEnrichCopiesDerivedIdentityToRequestAndOptions(t *testing.T) {
	...
	req := shinwayexecutor.Request{Payload: []byte(`{"messages":[{"role":"user","content":"hello"}]}`)}
	opts := shinwayexecutor.Options{
		OriginalRequest: req.Payload,
		SourceFormat:    sdktranslator.FormatOpenAI,
		Metadata:        map[string]any{shinwayexecutor.CallerScopeMetadataKey: "caller-a"},
	}
	...
	if _, exists := req.Metadata[shinwayexecutor.DerivedSessionIDMetadataKey]; exists {
...
func TestEnrichCarriesRequestPayloadIntoSelectionOptions(t *testing.T) {
	...
	_, enrichedOpts := Enrich(
		shinwayexecutor.Request{Payload: payload},
