...
func DerivedID(metadata map[string]any) string {
	...
	value, _ := metadata[shinwayexecutor.DerivedSessionIDMetadataKey].(string)
	return strings.TrimSpace(value)
}

// Enrich derives a session identity once and places it in both request and option metadata.
func Enrich(req shinwayexecutor.Request, opts shinwayexecutor.Options) (shinwayexecutor.Request, shinwayexecutor.Options) {
	payload := opts.OriginalRequest
	if len(payload) == 0 && len(req.Payload) > 0 {
		opts.OriginalRequest = bytes.Clone(req.Payload)
		payload = opts.OriginalRequest
	}
	if executionID := firstNormalizedMetadataID(shinwayexecutor.ExecutionSessionMetadataKey, opts.Metadata, req.Metadata); executionID != "" {
		req.Metadata = metadataWithValue(metadataWithoutKey(req.Metadata, shinwayexecutor.DerivedSessionIDMetadataKey), shinwayexecutor.ExecutionSessionMetadataKey, executionID)
		opts.Metadata = metadataWithValue(metadataWithoutKey(opts.Metadata, shinwayexecutor.DerivedSessionIDMetadataKey), shinwayexecutor.ExecutionSessionMetadataKey, executionID)
		return req, opts
	}
	req.Metadata = metadataWithoutKey(req.Metadata, shinwayexecutor.ExecutionSessionMetadataKey)
	opts.Metadata = metadataWithoutKey(opts.Metadata, shinwayexecutor.ExecutionSessionMetadataKey)
	if hasExplicitSession(opts.Headers, payload) {
		req.Metadata = metadataWithoutKey(req.Metadata, shinwayexecutor.DerivedSessionIDMetadataKey)
		opts.Metadata = metadataWithoutKey(opts.Metadata, shinwayexecutor.DerivedSessionIDMetadataKey)
		return req, opts
	}

	derivedID := firstNormalizedMetadataID(shinwayexecutor.DerivedSessionIDMetadataKey, opts.Metadata, req.Metadata)
	req.Metadata = metadataWithoutKey(req.Metadata, shinwayexecutor.DerivedSessionIDMetadataKey)
	opts.Metadata = metadataWithoutKey(opts.Metadata, shinwayexecutor.DerivedSessionIDMetadataKey)
	if derivedID == "" {
		callerScope := metadataString(opts.Metadata, shinwayexecutor.CallerScopeMetadataKey)
		if callerScope == "" {
			callerScope = metadataString(req.Metadata, shinwayexecutor.CallerScopeMetadataKey)
		}
	...
	req.Metadata = metadataWithValue(req.Metadata, shinwayexecutor.DerivedSessionIDMetadataKey, derivedID)
	opts.Metadata = metadataWithValue(opts.Metadata, shinwayexecutor.DerivedSessionIDMetadataKey, derivedID)
	return req, opts
}
