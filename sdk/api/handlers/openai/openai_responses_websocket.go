...
func websocketClosePayloadForUpstreamError(err error) (bool, []byte) {
	...
	errText := err.Error()
	if shinwayexecutor.IsUpstreamWebsocketReplayRequired(err) {
...
func (h *OpenAIResponsesAPIHandler) ResponsesWebsocket(c *gin.Context) {
	...
	for {
		...
		cliCtx = shinwayexecutor.WithDownstreamWebsocket(cliCtx)
		if nativeWebsocketPassthrough && requestRequiresCurrentUpstreamWebsocket {
			cliCtx = shinwayexecutor.WithRequiredUpstreamWebsocket(cliCtx)
		}
...
func responsesWebsocketHTTPReplayRequiredError() error {
	return shinwayexecutor.NewUpstreamWebsocketReplayRequiredError()
}
