...
func TestFileTokenStoreSaveExistingMetadataSetsFileAttributes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := auth.Attributes[shinwayauth.AttributePath]; got != path {
				t.Errorf("path attribute = %q, want %q", got, path)
			}
			if got := auth.Attributes[shinwayauth.AttributeSource]; got != path {
				t.Errorf("source attribute = %q, want %q", got, path)
			}
			if got := auth.Attributes[shinwayauth.AttributeSourceBackend]; got != shinwayauth.AuthSourceFile {
				t.Errorf("source backend attribute = %q, want %q", got, shinwayauth.AuthSourceFile)
			}
...
func TestFileTokenStoreListSkipsInvalidPluginSourceWeight(t *testing.T) {
	...
	parserCalled := false
	RegisterPluginAuthParser(fileStoreMultiAuthParserFunc(func(context.Context, pluginapi.AuthParseRequest) ([]*shinwayauth.Auth, bool, error) {
		parserCalled = true
		return []*shinwayauth.Auth{{ID: "plugin.json", Provider: "plugin"}}, true, nil
	}))
	t.Cleanup(func() {
...
func TestFileTokenStoreListExpandsPluginMultiAuths(t *testing.T) {
	...
	RegisterPluginAuthParser(fileStoreMultiAuthParserFunc(func(ctx context.Context, req pluginapi.AuthParseRequest) ([]*shinwayauth.Auth, bool, error) {
	...
	for _, auth := range auths {
		if !shinwayauth.IsPluginVirtualAuth(auth) {
			t.Fatalf("auth attributes = %#v, want plugin virtual marker", auth.Attributes)
		}
		if auth.Attributes[shinwayauth.AttributeVirtualSource] != path {
			t.Fatalf("virtual_source = %q, want %q", auth.Attributes[shinwayauth.AttributeVirtualSource], path)
		}
		...
		if gotWeight := auth.Attributes[shinwayauth.AttributeWeight]; gotWeight != "3" {
...
func TestFileTokenStoreListAppliesSourceDisabledToPluginMultiAuths(t *testing.T) {
	...
	for _, auth := range auths {
		if !auth.Disabled || auth.Status != shinwayauth.StatusDisabled {
...
type fileStoreMultiAuthParserFunc func(context.Context, pluginapi.AuthParseRequest) ([]*shinwayauth.Auth, bool, error)

func (f fileStoreMultiAuthParserFunc) ParseAuth(context.Context, pluginapi.AuthParseRequest) (*shinwayauth.Auth, bool, error) {
	return nil, false, nil
}

func (f fileStoreMultiAuthParserFunc) ParseAuths(ctx context.Context, req pluginapi.AuthParseRequest) ([]*shinwayauth.Auth, bool, error) {
	return f(ctx, req)
}
