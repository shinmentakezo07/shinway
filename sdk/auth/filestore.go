...
type PluginAuthParser interface {
	ParseAuth(context.Context, pluginapi.AuthParseRequest) (*shinwayauth.Auth, bool, error)
}
...
type PluginMultiAuthParser interface {
	ParseAuths(context.Context, pluginapi.AuthParseRequest) ([]*shinwayauth.Auth, bool, error)
}
...
func (s *FileTokenStore) Save(ctx context.Context, auth *shinwayauth.Auth) (string, error) {
	if auth == nil {
		return "", fmt.Errorf("auth filestore: auth is nil")
	}
	if errWeight := shinwayauth.ValidateAuthWeight(auth); errWeight != nil {
	...
	auth.Attributes[shinwayauth.AttributePath] = path
	auth.Attributes[shinwayauth.AttributeSource] = path
	auth.Attributes[shinwayauth.AttributeSourceBackend] = shinwayauth.AuthSourceFile
...
func (s *FileTokenStore) List(ctx context.Context) ([]*shinwayauth.Auth, error) {
	dir := s.baseDirSnapshot()
	if dir == "" {
		return nil, fmt.Errorf("auth filestore: directory not configured")
	}
	entries := make([]*shinwayauth.Auth, 0)
...
func (s *FileTokenStore) readAuthFiles(path, baseDir string) ([]*shinwayauth.Auth, error) {
	...
	if errWeight := shinwayauth.ValidateAuthWeight(&shinwayauth.Auth{Metadata: metadata}); errWeight != nil {
	...
	if parser := currentPluginAuthParser(); parser != nil {
		...
		if errParse == nil && handled {
			...
			for index, auth := range auths {
				...
				if len(auths) > 1 {
					shinwayauth.MarkPluginVirtualAuth(auth, path, index)
				}
				...
				if disabled {
					auth.Disabled = true
					auth.Status = shinwayauth.StatusDisabled
					if auth.Metadata == nil {
						auth.Metadata = make(map[string]any)
					}
					auth.Metadata["disabled"] = true
				}
				if errWeight := shinwayauth.ApplyAuthWeightMetadata(auth, metadata); errWeight != nil {
					return nil, errWeight
				}
				shinwayauth.ApplyCustomHeadersFromMetadata(auth)
			}
			return auths, nil
		}
	}
	...
	disabled, _ := metadata["disabled"].(bool)
	status := shinwayauth.StatusActive
	if disabled {
		status = shinwayauth.StatusDisabled
	}
	auth := &shinwayauth.Auth{
		...
		Attributes: map[string]string{
			shinwayauth.AttributePath:          path,
			shinwayauth.AttributeSource:        path,
			shinwayauth.AttributeSourceBackend: shinwayauth.AuthSourceFile,
		},
		Metadata:         metadata,
	...
	shinwayauth.ApplyCustomHeadersFromMetadata(auth)
	return []*shinwayauth.Auth{auth}, nil
}

func (s *FileTokenStore) readAuthFile(path, baseDir string) (*shinwayauth.Auth, error) {
...
func parsePluginAuthFile(parser PluginAuthParser, req pluginapi.AuthParseRequest) ([]*shinwayauth.Auth, bool, error) {
	if parser == nil {
	...
	return []*shinwayauth.Auth{auth}, true, nil
}

func compactPluginAuths(auths []*shinwayauth.Auth) []*shinwayauth.Auth {
	if len(auths) == 0 {
		return nil
	}
	out := auths[:0]
...
func (s *FileTokenStore) resolveAuthPath(auth *shinwayauth.Auth) (string, error) {
	if auth == nil {
		return "", fmt.Errorf("auth filestore: auth is nil")
	}
	if auth.Attributes != nil {
