...
- The service builds the queue via `ensureAuthUpdateQueue`, using a buffered channel and a dedicated consumer goroutine (`consumeAuthUpdates`). The consumer drains bursts by looping through the backlog before reacquiring the select loop.
...
- Back-pressure is absorbed in two places:
  - The dispatch buffer coalesces repeated updates for the same credential until the consumer catches up.
  - The service channel capacity combined with the consumer drain loop ensures several bursts can be processed without oscillation.
